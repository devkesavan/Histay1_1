...
if($action == "hotel_list_wmi")
{
	...
	if($property_type_id != 0)
		$filter_qry .= "and ht.category_type_id='$property_type_id'";
	...
	if($sort_type == "sort_no")
	{
		$sort_qry = "order by ht.sort_no $sort";
	}
	elseif($sort_type == "rating")
	{
		$sort_qry = "order by rating $sort";
	}
	elseif($sort_type == "price")
	{
		$sort_qry = "order by cost $sort";
	}
	...
	$select_query = "SELECT ht.*,cm.city as city_name,min(rm.final_cost) as cost,min(rm.actual_cost) as actual_cost,ctt.name as property_type,
					(SELECT round(AVG(hr.rating),1) as rating from $hotel_rating_tables hr where hr.hotel_id=ht.id limit 1) as rating FROM $hotels_table ht 
					inner join $master_property_type_table ctt on ctt.id=ht.category_type_id 
					inner join $master_city_table cm on ht.city_id=cm.id 
...
if($action == "hotel_details")
{
	...
	while($data3 = $sel_query3->fetch_array())
	{
	    if(trim($data3['hotel_image']) != "")
            $hotel_image = $MAIN_URL."/".$hotel_img_dir."/large/".$data3['hotel_image'];
        else
            $hotel_image = $MAIN_URL."/".$hotel_img_dir."/default_hotel.jpg";
		$hi_arr[] = array("hotel_image_id" => $data3['id'], "hotel_image" => $hotel_image, "default_image" => $data3['default_image']);
	}
...
//Rating summary
if($action == "hotel_rating_summary")
{
	if($hotel_id != 0)
		$hotel_qry = "and hotel_id='$hotel_id'";
	else
	...
	$select_query = "select count(id) as tot_rating_count from ra_hotel_rating where status='A' $hotel_qry";
	$sel_query = $con->query($select_query) or die(mysqli_error($con));
	$data = $sel_query->fetch_array();
	$tot_rating_count = $data['tot_rating_count'];
	
	$arr = array();
	for($ri=5;$ri>=1;$ri--)
	{
    	$select_query = "select rating,count(id) as rating_count from ra_hotel_rating where status='A' $hotel_qry and rating='$ri' group by rating";
    	$sel_query = $con->query($select_query) or die(mysqli_error($con));
    	if($sel_query->num_rows > 0)
    	{
    	    $data = $sel_query->fetch_array();
    	    $avg_rating = round(5 / ($tot_rating_count / $data['rating_count']),1);
    	    $arr[] = array("rating" => $data['rating'], "rating_count" => $data['rating_count'], "avg_rating" => $avg_rating);
    	}
    	else
    	    $arr[] = array("rating" => $ri, "rating_count" => 0, "avg_rating" => 0);
	}
    
    $output = array("success" => 1, "hotel_rating_summary" => $arr);
	echo json_encode($output, JSON_UNESCAPED_SLASHES);
}
...
if($action == "members_login")
{
	...
	if($count == 1)
	{
		...
		$output = array("success" => 1,"member_id" => "$data[id]", "login_token" => "$login_token", "name" => "$data[name]", "email" => "$data[email]", "mobile" => "$data[mobile]","address" => "$data[address]","profile_photo" => "$profile_photo","status" => "$data[status]");
		echo json_encode($output, JSON_UNESCAPED_SLASHES);
...
if($action == "member_edit")
{
	...
	{
		$data = $sel_query->fetch_array();
		$output = array("success" => 1,"member_id" => "$data[id]","name" => "$data[name]", "email" => "$data[email]", "mobile" => "$data[mobile]","address" => "$data[address]","profile_photo" => "$profile_photo");
		echo json_encode($output, JSON_UNESCAPED_SLASHES);
	}
	else
	{
...
if($action == "member_update")
{
	...
	if($count == 1)
	{
		$data = $sel_query->fetch_array();
		//update Edit Profile
		$upd_query = "update $members_table set name='$name',address='$address',email='$email',mobile='$mobile' where login_token='$login_token'";
		$result = $con->query($upd_query) or die(mysqli_error($con));
		$output = array("success" => 1);
		echo json_encode($output, JSON_UNESCAPED_SLASHES);
	}
...
if($action == "hotel_rooms")
{
	...
	{
		//Room Rates
		$actual_cost = $final_cost = 0;
		$today = date('Y-m-d');
		$diff_days = round((strtotime($check_out) - strtotime($check_in)) / (60 * 60 * 24));
        for($d=0;$d<$diff_days;$d++)
        {
            $ck_date = date('Y-m-d', strtotime($check_in. " + $d days"));
    		$select_rate = "SELECT mrr.actual_cost,mrr.final_cost from $hotel_room_rates_table hrr 
                		    inner join $master_room_rates_table mrr on hrr.rate_id=mrr.id WHERE hrr.start_date<='$ck_date' 
                		    and hrr.end_date>='$ck_date' and hrr.status='A' and hrr.room_id='$data[id]'";
        ...
        //$actual_cost = round($actual_cost / $diff_days,2);
        //$final_cost = round($final_cost / $diff_days,2);
		...
		$hri_arr = "";
		$hri_arr = array();
		$select_query3 = "select DISTINCT * from $hotel_room_images_table where room_id='$data[id]' and status='A'";
		$sel_query3 =$con->query($select_query3) or die(mysqli_error($con));
		while($data3 = $sel_query3->fetch_array())
		{
		    if(trim($data3['room_image']) != "")
		        $room_image = $MAIN_URL."/".$room_img_dir."/large/".$data3['room_image'];
            else
                $room_image = $MAIN_URL."/".$room_img_dir."/default_room.jpg";
			$hri_arr[] = array("room_image_id" => $data3['id'], "room_image" => $room_image);
		}
		
		//Booked rooms count
		$select_query4 = "select id from $hotel_room_booking_table where room_id='$data[id]' and ((checkin_date<='$check_in' and checkout_date>'$check_in') or (checkin_date<='$check_in' and checkout_date>'$check_out')) and status='A'";
		$sel_query4 = $con->query($select_query4) or die(mysqli_error($con));
		$book_room_count = $sel_query4->num_rows;
		$available_room_count = $data['num_of_rooms'] - $book_room_count;
	    ...
	    $arr[] = array("room_id" => $data['id'],"hotel_id" => $data['hotel_id'], "name" => $data['name'], "short_description" => $data['short_description'], 
		     	...
		     	"max_people" => $data['max_people'], "min_people" => $data['min_people'], "num_of_rooms" => $data['num_of_rooms'],"available_room_count" => $available_room_count,
...
if($action == "hotel_search")
{
	...
	while($data = $sel_query->fetch_array())
	{
	    ...
	    $hotel_image = $MAIN_URL."/".$hotel_img_dir."/large/".$data['hotel_image'];
	    $arr[] = array("hotel_id" => $data['id'], "property_type" => $data['property_type'],"destination_id" => $data['destination_id'], "city_name" => $data['city_name'], "star" => $data['star'], "code" => $data['code'], "name" => $data['name'], "slug" => $data['slug'], "short_description" => $data['short_description'], 
	    	"description" => $data['description'], "address" => $data['address'], "phone_number" => $data['phone_number'], "email_id" => $data['email_id'], 
...
if($action == "member_google_login")
{
	...
	else
	{
		...
		$upd_query = "update $members_table set login_token='$login_token' where id='$data[id]'";
		$result = $con->query($upd_query) or die(mysqli_error($con));
...
if($action == "hotel_booking")
{
	...
	{*/
		...
		foreach($booked_rooms as $key => $value)
		{
			...
			if($result->num_rows)
			{
			    ...
			    $ins_query = "insert into $hotel_room_booking_table(booking_id,checkin_date,checkout_date,room_id,room_no_id,rooms,adults,childerns,cost,created_at,created_by,status)
		    				Values('$insert_id','$checkin_date','$checkout_date','$room_id','$room_no_id','$rooms','$adults','$childerns','$cost','$created_at','$created_by','$book_room_status')";
		...
		$output = array("success" => 1,"booking_id" => $insert_id,"booking_code" => $booking_code);
	/*}
	else
	{
		$output = array("success" => -1);
...
if($action == "hotel_booking_confirm")
{
	$current_date = date('Y-m-d');
	...
	$qry = "update $hotel_booking_table set payment_status='$payment_status' where id='$booking_id'";
	$con->query($qry) or die(mysqli_error($con));
	
	$qry = "update $hotel_room_booking_table set status='A' where booking_id='$booking_id'";
	$con->query($qry) or die(mysqli_error($con));
	
	$select_query = "SELECT * from $hotel_booking_table where id='$booking_id'";
	$result = $con->query($select_query) or die(mysqli_error($con));
	$data = $result->fetch_array();
	$booking_code = $data['booking_code'];

	$output = array("success" => 1,"booking_id" => $booking_id,"booking_code" => $booking_code,"customer_name" => $data['first_name'],"customer_email" => $data['email']);
	
	echo json_encode($output, JSON_UNESCAPED_SLASHES);
}
if($action == "hotel_booking_confirm_old")
{
	$current_date = date('Y-m-d');
	...
	$qry = "update $hotel_booking_table set payment_status='PD' where id='$booking_id'";
	$con->query($qry) or die(mysqli_error($con));
	...
	{*/
		foreach($booked_rooms as $key => $value)
		{
			...
			if($result->num_rows)
			{
			    ...
			    $ins_query = "insert into $hotel_room_booking_table(booking_id,checkin_date,checkout_date,room_id,room_no_id,rooms,adults,childerns,cost,created_at,created_by,status)
		    				Values('$booking_id','$checkin_date','$checkout_date','$room_id','$room_no_id','$rooms','$adults','$childerns','$cost','$created_at','$created_by','A')";
				$ins_result = $con->query($ins_query) or die(mysqli_error($con));	
				$booking_room_id = $con->insert_id;	
			}
		}
		...
		$output = array("success" => 1,"booking_id" => $booking_id,"booking_code" => $booking_code);
	/*}
	else
	{
		$output = array("success" => -1);
...
if($action == "booking_history_list")
{
	...
	while($data = $sel_query->fetch_array())
	{
	    ...
	    $arr[] = array("booking_id" => $data['id'],"booking_code" => $data['booking_code'],"payment_status" => $data['payment_status'], "hotel_name" => $data['hotel_name'], "hotel_image" => $hotel_image,
