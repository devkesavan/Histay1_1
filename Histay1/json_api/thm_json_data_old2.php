...
if($action == "hotel_list_wmi")
{
	...
	$select_query = "SELECT ht.*,cm.city as city_name,min(rm.final_cost) as cost FROM $hotels_table ht 
					inner join $master_city_table cm on ht.city_id=cm.id 
	...
	while($data = $sel_query->fetch_array())
	{
	    ...
	    $arr[] = array("hotel_id" => $data['id'], "destination_id" => $data['destination_id'], "city_name" => $data['city_name'], "star" => $data['star'], "code" => $data['code'], "name" => $data['name'], "slug" => $data['slug'], "short_description" => $data['short_description'], 
	    	"description" => $data['description'], "address" => $data['address'], "phone_number" => $data['phone_number'], "email_id" => $data['email_id'], 
	    	"map" => $data['map'],"rating" => $rdata['rating'],"review_count" => $rdata['review_count'],"booking_count" => $booking_count,"cost" => $data['cost'],"hotel_images" => $hi_arr);
	}
...
if($action == "hotel_details")
{
    ...
    $arr = array("hotel_id" => $data['id'], "destination_id" => $data['destination_id'], "city_name" => $data['city_name'], "star" => $data['star'], "code" => $data['code'], "name" => $data['name'], "slug" => $data['slug'], "short_description" => $data['short_description'], 
	    	"description" => $data['description'], "address" => $data['address'], "phone_number" => $data['phone_number'], "email_id" => $data['email_id'], 
	    	"map" => $data['map'],"cost" => $data['cost'],"rating" => $rdata['rating'],"review_count" => $rdata['review_count'],"hotel_facilities" => $hf_arr, "hotel_images" => $hi_arr);
...
if($action == "coupons_code_disp")
{
	...
	while($data = $sel_query->fetch_array())
	{
		$arr[] = array("coupon_id" => $data['id'],"coupon_code" => $data['coupon_code'],"name" => $data['name'],"description" => $data['description'],"discount_type" => $data['discount_type'], "discount_value" => $data['discount_value'], "image" => $data['image']);
	}
...
if($action == "hotel_addons")
{
	...
	if($hotel_id != 0)
		$filter_qry = "and hotel_id='$hotel_id'";
	else
		$filter_qry = "";
	
	$arr = array();
	$select_query = "select * from $hotel_addons_table where status='A' $filter_qry $sort_qry $limit_qry";
	$sel_query =$con->query($select_query) or die(mysqli_error($con));
	while($data = $sel_query->fetch_array())
	{
		$image = $MAIN_URL."/".$addon_img_dir."/small/".$data['image'];
	    $arr[] = array("addon_id" => $data['id'], "hotel_id" => $data['hotel_id'],"name" => $data['name'], "short_description" => $data['short_description'], "image" => $image, "amount" => $data['amount']);
	}
