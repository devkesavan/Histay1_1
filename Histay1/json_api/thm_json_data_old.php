...
if($action == "hotel_list")
{
	...
	$select_query = "SELECT ht.*,cm.city as city_name,hi.hotel_image as hotel_image,round(AVG(hr.rating),1) as rating,COUNT(DISTINCT hr.id) as review_count,COUNT(DISTINCT hb.id) as booking_count,min(rm.final_cost) as cost FROM $hotels_table ht 
					...
					inner join $hotel_rooms_tables rm on rm.hotel_id=ht.id where hi.default_image='Y' group by ht.id $sort_qry $limit_qry";
	$sel_query =$con->query($select_query) or die(mysqli_error($con));
	while($data = $sel_query->fetch_array())
	{
...
if($action == "hotel_details")
{
	...
	//Hotel List
	$select_query = "SELECT ht.*,cm.city as city_name,min(rm.final_cost) as cost,round(AVG(hr.rating),1) as rating,COUNT(DISTINCT hr.id) as review_count FROM $hotels_table ht 
					inner join $master_city_table cm on ht.city_id=cm.id 
					inner join $hotel_rooms_tables rm on rm.hotel_id=ht.id 
					inner join $hotel_rating_tables hr on ht.id=hr.hotel_id where ht.status='A' $hotel_qry $slug_qry $sort_qry $limit_qry";
	$sel_query =$con->query($select_query) or die(mysqli_error($con));
	...
	{
		$hi_arr[] = array("hotel_image_id" => $data3['id'], "hotel_image" => $data3['hotel_image'], "default_image" => $data3['default_image']);
	}

    $arr = array("hotel_id" => $data['id'], "destination_id" => $data['destination_id'], "city_name" => $data['city_name'], "star" => $data['star'], "code" => $data['code'], "name" => $data['name'], "slug" => $data['slug'], "short_description" => $data['short_description'], 
	    	"description" => $data['description'], "address" => $data['address'], "phone_number" => $data['phone_number'], "email_id" => $data['email_id'], 
	    	"map" => $data['map'],"cost" => $data['cost'],"rating" => $data['rating'],"review_count" => $data['review_count'],"hotel_facilities" => $hf_arr, "hotel_images" => $hi_arr);
...
if($action == "hotel_slider")
{
	...
	while($data = $sel_query->fetch_array())
	{
	    $arr[] = array("slider_id" => $data['id'], "title" => $data['title'], "text1" => $data['text1'], "text2" => $data['text2'], "slide_image" => $data['slide_image'], "page_link" => $data['page_link'], "sort_no" => $data['sort_no']);
	}
...
if($action == "hotel_page")
{
	...
	$arr = array();
	$select_query = "select * from $pages_table where status='A' $sort_qry $limit_qry";
	$sel_query =$con->query($select_query) or die(mysqli_error($con));
	while($data = $sel_query->fetch_array())
	{
	    $arr[] = array("page_id" => $data['id'], "name" => $data['name'], "title" => $data['title'], "slug" => $data['slug'], "main_content" => $data['main_content'], "parent_page" => $data['parent_page']);
	}
...
if($action == "hotel_menu")
{
	...
	while($data = $sel_query->fetch_array())
	{
	    $arr[] = array("menu_id" => $data['id'], "name" => $data['name'], "title" => $data['title'], "item_type" => $data['item_type'], "url" => $data['url'], "main_menu" => $data['main_menu'], "footer_menu" => $data['footer_menu']);
	}
...
if($action == "coupons_code")
{
	...
	$arr = array();
	$select_query = "SELECT cc.* FROM $coupons_code_table cc inner join $coupons_code_hotel_table hc on cc.id=hc.coupon_id where cc.coupon_code='$coupon_code' $hotel_qry $val_qry $date_qry";
	$sel_query =$con->query($select_query) or die(mysqli_error($con));
...
if($action == "members_signup")
{
	...
	$signup_type = "W";
	$insert_query = "insert into $members_table(name,email,address,mobile,password,signup_type,created_at,status) 
...
{
	$select_query = "select id,name,mobile,email,email_verified,status from $members_table where email='$email' and password='$password' and status='A'";
	$sel_query =$con->query($select_query) or die(mysqli_error($con));
	$count = mysqli_num_rows($sel_query);
	if($count == 1)
	{
		...
		$output = array("success" => 1,"member_id" => "$data[id]", "login_token" => "$login_token", "name" => "$data[name]", "email" => "$data[email]","mobile" => "$data[mobile]", "status" => "$data[status]");
		echo json_encode($output, JSON_UNESCAPED_SLASHES);
...
if($action == "hotel_rooms")
{
	...
	while($data= $sel_query->fetch_array())
	{
	    ...
	    $arr[] = array("room_id" => $data['id'],"hotel_id" => $data['hotel_id'], "name" => $data['name'], "short_description" => $data['short_description'], 
		     	...
		     	"final_cost" => $data['final_cost'], "actual_cost" => $data['actual_cost'], "gst_per" => $data['gst_per'], "homepage" => $data['homepage'], 
...
if($action == "members_google_login")
{
	$signup_type = "G";
    ...
    {
    	$created_at = date('Y-m-d H:i:s');
    	$select_query = "select * from $members_table where status='A' and email='$email'";
    	$sel_query =$con->query($select_query) or die(mysqli_error($con));
    	$count = mysqli_num_rows($sel_query);
    	if($count == 0)
    	{
    		//Register
    		$otp_code = "1234";
    		$ins_query = "insert into $members_table(id_token,name,address,email,mobile,password,otp_code,email_verified,signup_type,created_at,status) 
    						values('$id_token','$name','$address','$email','$mobile','$password','$otp_code','Y','$signup_type','$created_at','A')";
			...
			$upd_query = "update $members_table set email_verified='Y',login_token='$login_token' where id='$reg_id'";
			$result = $con->query($upd_query) or die(mysqli_error($con));
...
if($action == "members_facebook_login")
{
	$signup_type = "F";

	// From URL to get webpage contents. 
    $url = "https://graph.facebook.com/v2.12/me?fields=name,first_name,last_name,email&access_token=$id_token"; 
...
if($action == "hotel_booking")
{
	...
	if($count < $data['num_of_rooms'])
	{
		$insert_query = "insert into $hotel_booking_table(booking_code,member_id,hotel_id,first_name,last_name,email,address,postal_code,city_id,state_id,
						...
						'$state_id','$country_id','$phone_number','$mobile_number','$coupon_code','$discount_type','$discount_value','$order_value','$tax_id',
...
//hotel booking view
if($action == "hotel_booking_view")
{
	$arr = array();
	$select_query = "SELECT hb.* ,ht.name as hotel_name,ct.city as city,st.state as state from $hotel_booking_table hb 
	inner join $hotels_table ht on ht.id=hb.hotel_id 
	inner join $master_city_table ct on ct.id=hb.city_id 
	inner join $master_state_table st on st.id=hb.state_id where hb.status='A'";
	$sel_query =$con->query($select_query) or die(mysqli_error($con));
	while($data = $sel_query->fetch_array())
	{
	     $arr[] = array("booking" => $data['id'], "hotel_name" => $data['hotel_name'], "city" => $data['city'], "state" => $data['state'], "order_total" => $data['order_total'], "checkin_date" => $data['checkin_date'], "checkout_date" => $data['checkout_date']);
	}
    
    $output = array("success" => 1, "hotel_booking_list" => $arr);
	echo json_encode($output, JSON_UNESCAPED_SLASHES);
}
