...
if(isset($_SESSION['admin_uid']))
{
	...
	if(isset($_POST['action']))
	{
		...
		if($_POST['action'] == 'ac_add')
		{
			...
			{
				$insert_query = "insert into $THIS_TABLE(hotel_id,name,short_description,amount,sort_no,created_at,created_by,status)
												values('$hotel_id','$name','$short_description','$sort_no','$amount','$created_at','$created_by','$status')";
            	...
            	{            		
            		...
            		$new_file_name = "addon_".$insert_id;
				...
				if($inst_res === true ){
					...
					$response['message'] = 'Addon Not Added!';
				}
			} else {
				$response['status'] = 'error';
				$response = 'Addon Title Already exists!';
			}
		...
		if($_POST['action'] == 'ac_edit')
		{
			...
			$update_sql = "update $THIS_TABLE set name='$name',short_description='$short_description',sort_no='$sort_no',amount='$amount',updated_at='$updated_at',updated_by='$updated_by' where id='$id'";
			...
			if($up_res === true) {
				...
				$response['status'] = 'error';
				$response['message'] = 'Addon are Not Updated!';
			}
			echo json_encode($response);
		}
		//viewdata fetch
	 ...
	 // 							WHERE ta.status!='E' AND ta.id='$id'";
		...
		if($_POST['action'] == 'ac_change_status')
		{
			if($_POST['id'])
			{
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Addon Status changed successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Addon Status NOT changed.';
				}
				echo json_encode($response);
			}
	  }
		...
		if($_POST['action'] == 'ac_delete')
		{
			if($_POST['id']){
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Addon Record deleted Successfuly...';
				} else {
					$response['status'] = 'error';
			...
			<script>
		 	...
		 	<div class="table-responsive">
		 	  <table class="table table-striped table-hover" id="save-stage" style="width:100%;">
		 		...
		 		<tbody>
		 		<?php
		 			$select_query =  "SELECT ht.id,ht.name as hotel_name,ad.* FROM $THIS_TABLE ad
		 			         			INNER JOIN $hotels_table ht ON ht.id=ad.hotel_id 
		 			         			WHERE ad.status!='E'";
		 			$result = $con->query($select_query) or die(mysqli_error($con));
		 			if($result->num_rows > 0){
		 				while($data = $result->fetch_array()) {
		 					...
		 					if($data['image'] != "")
		 						$img = "<img src='../$addon_img_dir/small/$data[image]' style='width:100px;'>";
		 					else
		 						$img = "[NO IMAGE]";
		 					...
		 					echo "<tr colspan='7'><td>No Addon found.</td></tr>";
		 				}
		 			?>
		 		</tbody>
		 	  </table>
									...
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										<div class="input-group-prepend">
										 ...
										 <input type="file" id="image" name="image" class="form-control" onchange="checkFile(this)" accept="image/*" oninput="image_pre.src=window.URL.createObjectURL(this.files[0])" /><br>
		...
		function checkFile(item){
			...
			// Check and restrict the file size to 32 KB.
				if ($(item).get(0).files[0].size > (10971000)) {
					alert('Failed!! Max allowed file size is 5 MB.');
					$(item).replaceWith($(item).val('').clone(true));
					$('#save').prop('disabled', true);
				}
				else {
		...
		$(document).ready(function (){
			...
			$(document).on('click','.edit', function(event){
				...
				success:function(data)
				{
					...
					$('#image_pre').attr("src", "<?php echo "../$addon_img_dir/small/"; ?>"+data.image).show;
					$('#id').val(id);
					$('#action').val('ac_edit');
					$('#save').val('Update');
					},error:function(xhr, ajaxOptions, thrownError){alert(xhr.responseText); ShowMessage("??? ?? ","fail");}
