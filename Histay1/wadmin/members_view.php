...
if(isset($_SESSION['admin_uid']))
{
	...
	$page_title = "Members View";
	$sub_page_title = "Members View";
	$msg_title = "Members View";
	...
	if(isset($_POST['action']))
	{
		...
		if($_POST['action'] == 'ac_add')
		{
			...
			{
				$insert_query = "insert into $THIS_TABLE(name,email,created_at,created_by,status)
		...
		if($_POST['action'] == 'ac_edit')
		{
			...
			$update_sql = "update $THIS_TABLE set parent_id='$parent_id',name='$name',description='$description',sort_no='$sort_no',featured_cat='$featured_cat',cat_image='$cat_image',updated_at='$updated_at',updated_by='$updated_by' where id='$id'";
		...
		//View Address
		if($_POST['action'] == 'ac_address_fetch')
		{
			extract($_POST);
		 			...
		 			if($result->num_rows > 0){
		 				while($data = $result->fetch_array()) {
		 					...
		 					if($data['email_verified'] == "Y") {
		 						$email_verified = '<span class="badge badge-success badge-shadow"> YES</span>';
		 					}else if($data['email_verified'] == "N"){
		 						$email_verified = '<span class="badge badge-danger badge-shadow"> NO</span>';
							}
		 					echo "<tr>
		 							<td> $data[name]</td>
		 							<td> $data[email]</td>
					...
					<div class="card">
						 <div class="card-header">
							...
							<div class="card-header-action">
								<a href="export_members.php"><button type="button" id="export" class="btn btn-success">Export</button></a>
							</div>
							&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							<div class="card-header-action">
					  ...
					  <div class="modal-dialog modal-dialog-centered" role="document">
						...
						<form id="register_form" method="POST" autocomplete="off">
						    <div class="modal-body">
								...
								<div class="form-group row mb-4">
									<label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">Email Verified</label>
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										 <p>
										 <label><input type="radio" id="email_verified_y" name="email_verified" value="Y" required /> YES</label>&nbsp&nbsp&nbsp
										 <label><input type="radio" id="email_verified_n" name="email_verified" value="N" required /> NO</label>
										 </p>
									  </div>
									</div>
								</div>
		...
		$(document).on('click','.view', function(event){
			...
			$.ajax({
				...
				success:function(data)
				{
				   ...
				   $('#email_verified_y').attr('checked',false);
				   $('#email_verified_n').attr('checked',false);
				   if(data.email_verified == "N")
				   		$('#email_verified_n').attr('checked',true);
				   else if(data.email_verified == "Y")
						$('#email_verified_y').attr('checked',true);
		...
		$(document).on('click','.view1', function(event){
			...
			$.ajax({
				...
				success:function(data)
				{
				   ...
				   $('.modal-title').html('<i class="far fa-eye"></i>Members');
				   $('#data_div').html(data);
