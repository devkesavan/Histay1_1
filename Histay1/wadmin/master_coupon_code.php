...
if(isset($_SESSION['admin_uid']))
{
	...
	$sub_page_title = "Coupon Code";
	$msg_title = "Coupon Code";
	$THIS_TABLE =$coupons_code_table;
	...
	$img_sizes = array(350=>260,1400=>1040);
	...
	if(isset($_POST['action']))
	{
		...
		if($_POST['action'] == 'ac_add')
		{
			...
			{
				 $insert_query = "insert into $THIS_TABLE(coupon_code,name,description,discount_value,discount_type,display_on_offers,one_time_coupons,min_value,max_discount,valid_from,valid_to,sort_no,created_at,created_by,status)
				 values('$coupon_code','$name','$description','$discount_value','$discount_type','$display_on_offers','$one_time_coupons','$min_value','$max_discount','$valid_from','$valid_to','$sort_no','$created_at','$created_by','$status')";
            	...
            	{            		
            		...
            		$new_file_name = "coupon_".$insert_id;
				...
				if($inst_res === true ){
					...
					$response['message'] = 'Coupon Code Not Added!';
				}
			} else {
				$response['status'] = 'error';
				$response = 'Coupon Code Title Already exists!';
			}
		...
		if($_POST['action'] == 'ac_edit')
		{
			...
			$update_sql = "update $THIS_TABLE set coupon_code='$coupon_code',name='$name',description='$description',discount_value='$discount_value',discount_type='$discount_type',display_on_offers='$display_on_offers',one_time_coupons='$one_time_coupons',min_value='$min_value',max_discount='$max_discount',valid_from='$valid_from',valid_to='$valid_to',sort_no='$sort_no',updated_at='$updated_at',updated_by='$updated_by' where id='$id'";
			...
			if($up_res === true) {
				...
				$response['status'] = 'error';
				$response['message'] = 'Coupon Code are Not Updated!';
			}
			echo json_encode($response);
		}
		//viewdata fetch
	 ...
	 // 							WHERE ta.status!='E' AND ta.id='$id'";
		...
		if($_POST['action'] == 'ac_change_status')
		{
			if($_POST['id'])
			{
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Coupon Code Status changed successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Coupon Code Status NOT changed.';
				}
				echo json_encode($response);
			}
	  }
		...
		if($_POST['action'] == 'ac_delete')
		{
			if($_POST['id']){
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Coupon Code Record deleted Successfuly...';
				} else {
					$response['status'] = 'error';
		 			...
		 			if($result->num_rows > 0){
		 				while($data = $result->fetch_array()) {
		 					...
		 					if($data['discount_type'] == "FIX") {
		 						...
		 						$discount_type = '<span class="badge badge-primary badge-shadow">PERCENTAGE</span>';
		 					}
		 					if($data['one_time_coupons'] == "Y") {
		 						$one_time_coupons = '<span class="badge badge-danger badge-shadow">YES</span>';
		 					}else if($data['one_time_coupons'] == "N"){
		 						$one_time_coupons = '<span class="badge badge-primary badge-shadow">YES</span>';
		 					}
		 					echo "<tr>
		 							<td>$data[coupon_code]</td>
		 							<td>$data[name]</td>
									<td>$discount_type</td>
									<td>$one_time_coupons</td>
		 					...
		 					echo "<tr colspan='7'><td>No Coupon Code found.</td></tr>";
		 				}
		 			?>
		 		</tbody>
		 	  </table>
					  ...
					  <div class="modal-dialog modal-dialog-centered" role="document">
						...
						<form id="register_form" method="POST" autocomplete="off">
						    <div class="modal-body">
								...
								<div class="form-group row mb-4">
									<label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">Display On Offers</label>
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										 <label><input type="radio" id="display_on_offers_y" name="display_on_offers" value="Y" required /> YES</label>&nbsp&nbsp&nbsp
										 <label><input type="radio" id="display_on_offers_n" name="display_on_offers" value="N" required /> NO</label>
										 </p>
									  </div>
									</div>
								</div>
								<div class="form-group row mb-4">
									<label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">One Time Coupons</label>
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										 <p>
										 <label><input type="radio" id="one_time_coupons_y" name="one_time_coupons" value="Y" required /> YES</label>&nbsp&nbsp&nbsp
										 <label><input type="radio" id="one_time_coupons_n" name="one_time_coupons" value="N" required /> NO</label>
										 </p>
									  </div>
									</div>
								</div>
									...
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										<div class="input-group-prepend">
										 ...
										 <input type="text" id="min_value" name="min_value" class="form-control" /> 
									  </div>
									</div>
								</div>
								<div class="form-group row mb-4">
									<label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">Max Discount</label>
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										<div class="input-group-prepend">
										  <div class="input-group-text"> <i class="fas fa-map-pin"></i> </div>
								...
								<div class="form-group row mb-4">
									<label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">Valid From</label>
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										<div class="input-group-prepend">
											<div class="input-group-text"> <i class="fas fa-map-marker-alt"></i> </div>
								...
								<div class="form-group row mb-4">
									<label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">Valid To</label>
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										<div class="input-group-prepend">
										  <div class="input-group-text"> <i class="fas fa-map-pin"></i> </div>
								...
								<div class="form-group row mb-4">
									...
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										...
										<input type="file" id="image" name="image" class="form-control" onchange="checkFile(this)" accept="image/*" oninput="coupon_image_pre.src=window.URL.createObjectURL(this.files[0])"/><br>
										<img id="coupon_image_pre" src="" class="img-fluid" style="width:150px;" />
										</div>
									</div>
								</div>
	...
	<script>
		...
		$(document).on('click','.edit', function(event){
			...
			success:function(data)
			{
				...
				$('#display_on_offers_y').attr('checked',false);
				$('#display_on_offers_n').attr('checked',false);
				if(data.display_on_offers == "N")
					$('#display_on_offers_n').attr('checked',true);
				else if(data.display_on_offers == "Y")
					$('#display_on_offers_y').attr('checked',true);



				$('#one_time_coupons_y').attr('checked',false);
				$('#one_time_coupons_n').attr('checked',false);
				if(data.one_time_coupons == "Y")
					$('#one_time_coupons_y').attr('checked',true);
				else if(data.one_time_coupons == "N")
					$('#one_time_coupons_n').attr('checked',true);
				$('#min_value').val(data.min_value);
				$('#max_discount').val(data.max_discount);
				
				$('#valid_from').val(data.valid_from);
				//$('#valid_from').val($.format.date(data.valid_from, "dd-mm-yyyy"));
				$('#valid_to').val(data.valid_to);
				$('#sort_no').val(data.sort_no);
				$('#coupon_image_pre').attr("src", "<?php echo "../$coupon_img_dir/small/"; ?>"+data.image).show;
				$('#id').val(id);
				$('#action').val('ac_edit');
				$('#save').val('Update');
				},error:function(xhr, ajaxOptions, thrownError){alert(xhr.responseText); ShowMessage("??? ?? ","fail");}
