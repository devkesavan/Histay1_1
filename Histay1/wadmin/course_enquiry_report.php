...
if(isset($_SESSION['admin_uid']))
{
	...
	$page_title = "Course Enquires";
	$sub_page_title = "Course Enquires";
	$msg_title = "Course Enquires";
	$THIS_TABLE = $course_enquires_table;
					...
					<div class="col-sm-12 col-md-9">
					 <div class="input-group">
						...
						<select class="form-control" name="course_id" id="course_id">
							...
							<?php
						 	echo $select_query = "select id,name from $course_management_table where status!='E'";
                        	$result = $con->query($select_query) or die(mysqli_error($con));
		 		...
		 		$filter = "";
					if(isset($_POST['filter']))
					{
						if($_POST['from_date'] != "" && $_POST['to_date'] != "")
							$filter .= " and date(ce.created_at)>='$_POST[from_date]' and date(ce.created_at)<='$_POST[to_date]'";
						if($_POST['course_id'] != "")
							$filter .= " and ce.course_id='$_POST[course_id]' ";
					}

		 			$select_query = "SELECT cm.name as course_name,ce.* FROM $THIS_TABLE ce INNER JOIN $course_management_table cm ON 
		 			                 cm.id=ce.course_id WHERE ce.status!='E' $filter";
		 			$result = $con->query($select_query) or die(mysqli_error($con));
		 			if($result->num_rows > 0){
		 				while($data = $result->fetch_array()) {
		 					if($data['status'] == "A") {
