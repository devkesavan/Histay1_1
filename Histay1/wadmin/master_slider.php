...
if(isset($_SESSION['admin_uid']))
{
	...
	$page_title = "Master Slider";
	$sub_page_title = "Master Slider";
	$msg_title = "Master Slider";
	$THIS_TABLE = $master_slider_table;
	...
	$img_sizes = array(300=>300,2000=>1000);
	$disp_img_sizes = "2000 x 1000";
	...
	if(isset($_POST['action']))
	{
		...
		if($_POST['action'] == 'ac_add')
		{
			...
			{
				$insert_query = "insert into $THIS_TABLE(title,text1,text2,page_link,sort_no,created_at,created_by,status)
												values('$title','$text1','$text2','$page_link','$sort_no','$created_at','$created_by','$status')";
            	...
            	{            		
            		...
            		$new_file_name = "slider_".$insert_id;
            		...
            		if(empty($errors)==true)
            		{
            			...
            			foreach ($img_sizes as $w => $h)
            			{
                            ...
                            {
                                $folder = "large";
                                list($width, $height) = getimagesize($_FILES['slide_image']['tmp_name']);
                                $h = $w / ($width / $height);
                            }
                            $img_path = "../".$slider_img_dir;
                            $files[] = resize('slide_image', $w, $h, $new_file_name, $img_path, $folder);
            			}
            			$ud_query = "update $THIS_TABLE set slide_image='$new_up_file_name' where id='$insert_id'";
            	...
            	if($inst_res === true ){
					...
					$response['message'] = 'Slider Not Added!';
				}
			} else {
				$response['status'] = 'error';
				$response = 'Slider Title Already exists!';
			}
		...
		if($_POST['action'] == 'ac_edit')
		{
			...
			$update_sql = "update $THIS_TABLE set title='$title',text1='$text1',text2='$text2',page_link='$page_link',sort_no='$sort_no',updated_at='$updated_at',updated_by='$updated_by' where id='$id'";
        	...
        	{            		
        		...
        		if(empty($errors)==true)
        		{
        			...
        			$ud_query = "update $THIS_TABLE set slide_image='$new_up_file_name' where id='$id'";
        			$con->query($ud_query) or die(mysqli_error($con));
        		}
        		else
        		{
			...
			if($up_res === true) {
				...
				$response['status'] = 'error';
				$response['message'] = 'Slider are Not Updated!';
			}
			echo json_encode($response);
		}
		//viewdata fetch
		...
		if($_POST['action'] == 'ac_change_status')
		{
			if($_POST['id'])
			{
				...
				if($res === true ){
					...
					$response['status'] = 'error';
					$response['message'] = 'Slider Status NOT changed.';
				}
				echo json_encode($response);
			}
	  }
		...
		if($_POST['action'] == 'ac_delete')
		{
			if($_POST['id']){
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Slider deleted Successfuly...';
				} else {
					$response['status'] = 'error';
		 		...
		 		<?php
		 			...
		 			if($result->num_rows > 0){
		 				while($data = $result->fetch_array()) {
		 					...
		 					if($data['slide_image'] != "")
		 						$slide_image = "<img src='../$slider_img_dir/small/$data[slide_image]' style='width:100px;'>";
		 					else
		 						$slide_image = "[NO IMAGE]";
							echo "<tr>
		 							<td> $data[title]</td>
									<td> $data[text1]</td>
		 							<td> $data[text2]</td>
		 							<td> $slide_image</td>
									<td> <a href='$data[page_link]' target='_blank'><span class='badge badge-success badge-shadow'>LINK</span></a></td>
									<td> $data[sort_no]</td>
		 					...
		 					echo "<tr colspan='7'><td> Varient Not found.</td></tr>";
		 				}
		 			?>
		 		</tbody>
		 	  </table>
					  ...
					  <div class="modal-dialog modal-dialog-centered" role="document">
						...
						<form id="register_form" method="POST" autocomplete="off">
						    <div class="modal-body">
								...
								<div class="form-group row mb-4">
									<label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">Text1</label>
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										<div class="input-group-prepend">
										  <div class="input-group-text"> <i class="fas fa-map-marker-alt"></i> </div>
								...
								<div class="form-group row mb-4">
									<label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">Text2</label>
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										<div class="input-group-prepend">
										  <div class="input-group-text"> <i class="fas fa-map-marker-alt"></i> </div>
									...
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										<div class="input-group-prepend">
										 ...
										 <input type="file" id="slide_image" name="slide_image" class="form-control" onchange="checkFile(this)" accept="image/*" oninput="slide_image_pre.src=window.URL.createObjectURL(this.files[0])" required /><br>
										 <img id="slide_image_pre" src="" class="img-fluid" style="width:150px;" />
									  </div>
									</div>
								</div>
								<div class="form-group row mb-4">
									<label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">Page Link</label>
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										<div class="input-group-prepend">
										  <div class="input-group-text"> <i class="fas fa-map-marker-alt"></i> </div>
										</div>
										<input type="text" id="page_link" name="page_link" class="form-control" required />
									  </div>
									</div>
								</div>
								<div class="form-group row mb-4">
	...
	<script>
		...
		$(document).ready(function (){
			...
			$('#usermodal').click(function(){
				...
				$('.modal-title').html('<i class="fas fa-list-alt"></i> Add Country');
				$('#save').val('Save');
		...
		$(document).on('click','.edit', function(event){
			...
			success:function(data)
			{
				...
				$('#slide_image_pre').attr("src", "<?php echo "../$slider_img_dir/small/"; ?>"+data.slide_image).show;
