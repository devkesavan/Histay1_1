...
if(isset($_SESSION['admin_uid']))
{
	...
	if(isset($_POST['action']))
	{
		...
		if($_POST['action'] == 'ac_add')
		{
			...
			{
				$insert_query = "insert into $THIS_TABLE(name,title,item_type,page_id,url,main_menu,footer_menu,sort_no,created_at,created_by,status)
												values('$name','$title','$item_type','$page_id','$url','$main_menu','$footer_menu','$sort_no','$created_at','$created_by','$status')";
				$inst_res = $con->query($insert_query) or die(mysqli_error($con));

				if($inst_res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Menus Added successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Menus Not Added!';
				}
			} else {
				$response['status'] = 'error';
				$response = 'Menus Title Already exists!';
			}
		...
		if($_POST['action'] == 'ac_edit')
		{
			...
			$update_sql = "update $THIS_TABLE set name='$name',title='$title',item_type='$item_type',page_id='$page_id',url='$url',main_menu='$main_menu',footer_menu='$footer_menu',sort_no='$sort_no',updated_at='$updated_at',updated_by='$updated_by' where id='$id'";
			$up_res = $con->query($update_sql);
			if($up_res === true) {
				$response['status'] = 'success';
				$response['message'] = 'Menus updated successfully!';
			} else {
				$response['status'] = 'error';
				$response['message'] = 'Menus are Not Updated!';
			}
			echo json_encode($response);
		}
		//viewdata fetch
	 ...
	 // 							WHERE ta.status!='E' AND ta.id='$id'";
		...
		if($_POST['action'] == 'ac_change_status')
		{
			if($_POST['id'])
			{
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Hotel Rating Status changed successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Hotel Rating Status NOT changed.';
				}
				echo json_encode($response);
			}
	  }
		...
		if($_POST['action'] == 'ac_delete')
		{
			if($_POST['id']){
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Hotel Rating Record deleted Successfuly...';
				} else {
					$response['status'] = 'error';
		 		...
		 		<?php
		 			...
		 			if($result->num_rows > 0){
		 				while($data = $result->fetch_array()) {
		 					...
		 					if($data['item_type'] == "P") {
		 						$item_type = '<span class="badge badge-primary badge-shadow">PAGE</span>';
		 					}else if($data['item_type'] == "U"){
		 						$item_type = '<span class="badge badge-primary badge-shadow">URL</span>';
							}
							if($data['main_menu'] == "Y") {
		 						$main_menu = '<span class="badge badge-success badge-shadow">YES</span>';
		 					}else if($data['main_menu'] == "N"){
		 						$main_menu = '<span class="badge badge-danger badge-shadow">NO</span>';
							}
							if($data['footer_menu'] == "Y") {
		 					...
		 					echo "<tr colspan='7'><td>No Hotel Rating found.</td></tr>";
		 				}
		 			?>
		 		</tbody>
		 	  </table>
					  ...
					  <div class="modal-dialog modal-dialog-centered" role="document">
						...
						<form id="register_form" method="POST" autocomplete="off">
						    <div class="modal-body">
								...
								<div class="form-group row mb-4">
									<label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">Item Type</label>
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										 <p>
										 <label><input type="radio" id="item_type_p" name="item_type" value="P" required /> PAGE</label>&nbsp&nbsp&nbsp
										 <label><input type="radio" id="item_type_n" name="item_type" value="U" required /> URL</label>
										 </p>
									  </div>
									</div>
								</div>
									...
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										...
										<option value=''>Select Tax</option>
										 <?php
											$select_query = "select * from $pages_table where status!='E'";
	                                    	$result = $con->query($select_query) or die(mysqli_error($con));
	                                    	while($data = $result->fetch_array())
	                                    	{
								...
								<div class="form-group row mb-4">
									<label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">Main Menu</label>
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										 <p>
										 <label><input type="radio" id="main_menu_y" name="main_menu" value="Y" required /> YES</label>&nbsp&nbsp&nbsp
										 <label><input type="radio" id="main_menu_n" name="main_menu" value="N" required /> NO</label>
										 </p>
									  </div>
									</div>
								</div>
	...
	<script>
		...
		$(document).on('click','.edit', function(event){
			...
			success:function(data)
			{
				...
				$('#item_type_p').attr('checked',false);
				$('#item_type_u').attr('checked',false);
				if(data.item_type == "P")
					$('#item_type_p').attr('checked',true);
				else if(data.item_type == "U")
					$('#item_type_u').attr('checked',true);
				$('#page_id').val(data.page_id);
				$('#url').val(data.url);
				$('#main_menu_y').attr('checked',false);
				$('#main_menu_n').attr('checked',false);
				if(data.main_menu == "Y")
					$('#main_menu_y').attr('checked',true);
				else if(data.main_menu == "N")
					$('#main_menu_n').attr('checked',true);
