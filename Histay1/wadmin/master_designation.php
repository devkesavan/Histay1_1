...
if(isset($_SESSION['admin_uid']))
{
	...
	$sub_page_title = "Designation";
	$msg_title = "Designation";
	$THIS_TABLE = $master_designation_table;
	...
	if(isset($_POST['action']))
	{
		...
		if($_POST['action'] == 'ac_add')
		{
			...
			{
				$insert_query = "insert into $THIS_TABLE(name,designation,created_at,created_by,status)
												values('$name','$designation','$created_at','$created_by','$status')";
				...
				if($inst_res === true ){
					...
					$response['message'] = 'Designation Not Added!';
				}
			} else {
				$response['status'] = 'error';
				$response = 'Designation Title Already exists!';
			}
		...
		if($_POST['action'] == 'ac_edit')
		{
			...
			$update_sql = "update $THIS_TABLE set name='$name',designation='$designation',updated_at='$updated_at',updated_by='$updated_by' where id='$id'";
			...
			if($up_res === true) {
				...
				$response['status'] = 'error';
				$response['message'] = 'Designation are Not Updated!';
			}
			echo json_encode($response);
		}
		//viewdata fetch
		...
		if($_POST['action'] == 'ac_change_status')
		{
			if($_POST['id'])
			{
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Designation Status changed successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Designation Status NOT changed.';
				}
				echo json_encode($response);
			}
	  }
		...
		if($_POST['action'] == 'ac_delete')
		{
			if($_POST['id']){
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Designation Record deleted Successfuly...';
				} else {
					$response['status'] = 'error';
