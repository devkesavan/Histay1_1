...
if(isset($_SESSION['admin_uid']))
{
	...
	$page_title = "Master Projects";
	$sub_page_title = "Master Projects";
	$msg_title = "Master Projects";
	$THIS_TABLE = $master_projects_table;
	...
	if(isset($_POST['action']))
	{
		...
		if($_POST['action'] == 'ac_add')
		{
			...
			{
				$insert_query = "insert into $THIS_TABLE(name,description,reference,start_date,end_date,dead_line_date,created_at,created_by,status)
												values('$name','$description','$reference','$start_date','$end_date','$dead_line_date','$created_at','$created_by','$status')";
				$inst_res = $con->query($insert_query) or die(mysqli_error($con));

				if($inst_res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Projects Added successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Projects Not Added!';
				}
			} else {
				$response['status'] = 'error';
				$response = 'Projects Title Already exists!';
			}
		...
		if($_POST['action'] == 'ac_edit')
		{
			...
			$update_sql = "update $THIS_TABLE set name='$name',description='$description',reference='$reference',
							start_date='$start_date',end_date='$end_date',dead_line_date='$dead_line_date',updated_at='$updated_at',updated_by='$updated_by' where id='$id'";
			$up_res = $con->query($update_sql);
			if($up_res === true) {
				$response['status'] = 'success';
				$response['message'] = 'Projects updated successfully!';
			} else {
				$response['status'] = 'error';
				$response['message'] = 'Projects are Not Updated!';
			}
			echo json_encode($response);
		}
		//viewdata fetch
		...
		if($_POST['action'] == 'ac_change_status')
		{
			if($_POST['id'])
			{
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Projects Status changed successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Projects Status NOT changed.';
				}
				echo json_encode($response);
			}
	  }
		...
		if($_POST['action'] == 'ac_delete')
		{
			if($_POST['id']){
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Projects Record deleted Successfuly...';
				} else {
					$response['status'] = 'error';
		 		...
		 		<?php
		 			...
		 			if($result->num_rows > 0){
		 				while($data = $result->fetch_array()) {
		 					...
		 					echo "<tr colspan='7'><td>No Projects found.</td></tr>";
		 				}
		 			?>
		 		</tbody>
		 	  </table>
					  ...
					  <div class="modal-dialog modal-dialog-centered" role="document">
						...
						<form id="register_form" method="POST" autocomplete="off">
						    <div class="modal-body">
								...
								<div class="form-group row mb-4">
									<label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">Deadline Date</label>
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										<div class="input-group-prepend">
										  <div class="input-group-text"> <i class="fas fa-map-pin"></i> </div>
										</div>
										<input type="date" id="dead_line_date" name="dead_line_date" class="form-control" />
									  </div>
									</div>
								</div>
	...
	<script>
		$(document).ready(function (){
			...
			$('#usermodal').click(function(){
				...
				$('.modal-title').html('<i class="fas fa-list-alt"></i> Add Projects');
				$('#save').val('Save');
			});
		...
		$(document).on('click','.edit', function(event){
			...
			success:function(data)
			{
				...
				$('#dead_line_date').val(data.dead_line_date);
				$('#id').val(id);
				$('#action').val('ac_edit');
				$('#save').val('Update');
				},error:function(xhr, ajaxOptions, thrownError){alert(xhr.responseText); ShowMessage("??? ?? ","fail");}
