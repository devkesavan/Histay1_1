...
if(isset($_SESSION['admin_uid']))
{
	...
	$page_title = "Hotel Class";
	$sub_page_title = "Hotel Class";
	$msg_title = "Hotel Class";
	$THIS_TABLE = $master_hotel_class_table;
	...
	if(isset($_POST['action']))
	{
		...
		if($_POST['action'] == 'ac_add')
		{
			...
			{
				$insert_query = "insert into $THIS_TABLE(class_no,name,description,created_at,created_by,status)
												values('$class_no','$name','$description','$created_at','$created_by','$status')";
		...
		if($_POST['action'] == 'ac_edit')
		{
			...
			$update_sql = "update $THIS_TABLE set class_no='$class_no',name='$name',description='$description',updated_at='$updated_at',updated_by='$updated_by' where id='$id'";
			$up_res = $con->query($update_sql);
			if($up_res === true) {
				$response['status'] = 'success';
				$response['message'] = 'Hotel Class updated successfully!';
			} else {
				$response['status'] = 'error';
				$response['message'] = 'Hotel Class are Not Updated!';
			}
			echo json_encode($response);
		}
		//viewdata fetch
	 ...
	 // 							WHERE ta.status!='E' AND ta.id='$id'";
		...
		if($_POST['action'] == 'ac_change_status')
		{
			if($_POST['id'])
			{
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Hotel Class Status changed successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Hotel Class Status NOT changed.';
				}
				echo json_encode($response);
			}
	  }
		...
		if($_POST['action'] == 'ac_delete')
		{
			if($_POST['id']){
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Hotel Class Record deleted Successfuly...';
				} else {
					$response['status'] = 'error';
		 		...
		 		<?php
		 			...
		 			if($result->num_rows > 0){
		 				while($data = $result->fetch_array()) {
		 					...
		 					echo "<tr colspan='7'><td>No Hotel Class found.</td></tr>";
		 				}
		 			?>
		 		</tbody>
		 	  </table>
						...
						<form id="register_form" method="POST" autocomplete="off">
						    <div class="modal-body">
								<div class="form-group row mb-4">
									<label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">Class No</label>
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										<div class="input-group-prepend">
										  <div class="input-group-text"> <i class="fas fa-map-marker-alt"></i></div>
										</div>
										<input type="number" id="class_no" name="class_no" class="form-control" />
									  </div>
									</div>
								</div>
								<div class="form-group row mb-4">
