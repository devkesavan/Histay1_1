...
if(isset($_SESSION['admin_uid']))
{
	...
	if(isset($_POST['action']))
	{
		...
		if($_POST['action'] == 'ac_add')
		{
			...
			{
				$insert_query = "insert into $THIS_TABLE(name,color,actual_cost,final_cost,sort_no,created_at,created_by,status)
												values('$name','$color','$actual_cost','$final_cost','$sort_no','$created_at','$created_by','$status')";
		...
		if($_POST['action'] == 'ac_edit')
		{
			...
			$update_sql = "update $THIS_TABLE set name='$name',color='$color',actual_cost='$actual_cost',final_cost='$final_cost',sort_no='$sort_no	',updated_at='$updated_at',updated_by='$updated_by' where id='$id'";
		 		...
		 		<?php
		 			...
		 			if($result->num_rows > 0){
		 				while($data = $result->fetch_array()) {
		 					...
		 					$color = "<span style='background:$data[color];padding: 0 25px;'>&nbsp;</span>";
		 					echo "<tr>
		 							<td>$data[name]</td>
		 							<td>$color</td>
									...
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										<div class="input-group-prepend">
										 ...
										 <input type="text" id="sort_no" name="sort_no" class="form-control" /> 
									  </div>
									</div>
								</div>
