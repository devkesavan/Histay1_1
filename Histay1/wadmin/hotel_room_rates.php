...
if(isset($_SESSION['admin_uid']))
{
	...
	$THIS_TABLE = $hotel_room_rates_table;
	...
	if(isset($_POST['action']))
	{
		...
		if($_POST['action'] == 'ac_add')
		{
			...
			/*$select_query = " select id from $THIS_TABLE where room_id='$room_id'and status!='E'";
			$result = $con->query($select_query) or die(mysqli_error($con));
			if($result->num_rows == 0)
			{*/
				$insert_query = "insert into $THIS_TABLE(room_id,rate_id,start_date,end_date,created_at,created_by,status)
												values('$room_id','$rate_id','$start_date','$end_date','$created_at','$created_by','$status')";
		...
		if($_POST['action'] == 'ac_edit')
		{
			...
			$update_sql = "update $THIS_TABLE set room_id='$room_id',rate_id='$rate_id',start_date='$start_date',end_date='$end_date',updated_at='$updated_at',updated_by='$updated_by' where id='$id'";
			...
			<script>
		 	...
		 	<div class="table-responsive">
		 	  <table class="table table-striped table-hover" id="save-stage" style="width:100%;">
		 		...
		 		<tbody>
		 		<?php
		 			$select_query = "SELECT ht.name as hotel_name,hr.name as room_name,mrr.name as rate_name,mrr.color,hrr.* FROM $THIS_TABLE hrr 
		 			INNER JOIN $hotel_rooms_tables hr ON hr.id=hrr.room_id
		 			INNER JOIN $hotels_table ht ON ht.id=hr.hotel_id
		 			INNER JOIN $master_room_rates_table mrr ON mrr.id=hrr.rate_id
					WHERE hrr.status!='E'";
		 			$result = $con->query($select_query) or die(mysqli_error($con));
		 			if($result->num_rows > 0){
		 				while($data = $result->fetch_array()) {
		 					...
		 					$color = "<span style='background:$data[color];padding: 0 15px;'>&nbsp;</span>";
		 					echo "<tr>
		 							<td><b>$data[hotel_name]</b> - $data[room_name]</td>
		 							<td>$data[rate_name] $color</td>
					  ...
					  <div class="modal-dialog modal-dialog-centered" role="document">
						...
						<form id="register_form" method="POST" autocomplete="off">
						    <div class="modal-body">
									...
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										...
										<select class="form-control" name="room_id" id="room_id" required/>
											...
											<?php
											$select_hquery = "select id,name as hotel_name from $hotels_table where status!='E'";
                                        	$hresult = $con->query($select_hquery) or die(mysqli_error($con));
                                        	while($hdata = $hresult->fetch_array())
                                        	{
                                        		echo "<optgroup label='$hdata[hotel_name]'>";

												$select_query = "select id,name as room_name from $hotel_rooms_tables where status!='E' and hotel_id='$hdata[id]'";
	                                        	$result = $con->query($select_query) or die(mysqli_error($con));
	                                        	while($data = $result->fetch_array())
	                                        	{
								...
								<div class="form-group row mb-4">
									<label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">Rate Name</label>
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										<div class="input-group-prepend">
										  <div class="input-group-text"> <i class="fas fa-map-marker-alt"></i> </div>
										</div>
										<select class="form-control" name="rate_id" id="rate_id" required/>
										<option value='0'>Select Rate</option>
										 <?php
											$select_query = "select id,name,color from $master_room_rates_table where status!='E'";
                                        	$result = $con->query($select_query) or die(mysqli_error($con));
                                        	while($data = $result->fetch_array())
                                        	{
