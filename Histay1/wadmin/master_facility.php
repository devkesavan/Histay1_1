...
if(isset($_SESSION['admin_uid']))
{
	...
	$sub_page_title = "Facilities";
	$msg_title = "Facilities";
	$THIS_TABLE = $master_facilities_table;
	...
	if(isset($_POST['action']))
	{
		...
		if($_POST['action'] == 'ac_add')
		{
			...
			{
				$insert_query = "insert into $THIS_TABLE(name,sort_no,created_at,created_by,status)
												values('$name','$sort_no','$created_at','$created_by','$status')";
            	...
            	{            		
            		...
            		$new_file_name = "facility_".$insert_id;
            		...
            		if(empty($errors)==true)
            		{
            			...
            			foreach ($img_sizes as $w => $h)
            			{
                            ...
                            $img_path = "../".$facility_img_dir;
                            $files[] = resize('icon_image', $w, $h, $new_file_name, $img_path, $folder);
            			}
            			$ud_query = "update $THIS_TABLE set icon_image='$new_up_file_name' where id='$insert_id'";
				...
				if($inst_res === true ){
					...
					$response['message'] = 'Facilities Added successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Facilities Not Added!';
				}
			} else {
				$response['status'] = 'error';
				$response = 'Facilities Title Already exists!';
			}
		...
		if($_POST['action'] == 'ac_edit')
		{
			...
			$update_sql = "update $THIS_TABLE set name='$name',sort_no='$sort_no',updated_at='$updated_at',updated_by='$updated_by' where id='$id'";
        	...
        	{            		
        		...
        		if(empty($errors)==true)
        		{
        			...
        			$ud_query = "update $THIS_TABLE set icon_image='$new_up_file_name' where id='$id'";
        			$con->query($ud_query) or die(mysqli_error($con));
        		}
        		else
        		{
			...
			if($up_res === true) {
				$response['status'] = 'success';
				$response['message'] = 'Facilities updated successfully!';
			} else {
				$response['status'] = 'error';
				$response['message'] = 'Facilities are Not Updated!';
			}
			echo json_encode($response);
		}
		//viewdata fetch
	 ...
	 // 							WHERE ta.status!='E' AND ta.id='$id'";
		...
		if($_POST['action'] == 'ac_change_status')
		{
			if($_POST['id'])
			{
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Facilities Status changed successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Facilities Status NOT changed.';
				}
				echo json_encode($response);
			}
	  }
		...
		if($_POST['action'] == 'ac_delete')
		{
			if($_POST['id']){
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Facilities Record deleted Successfuly...';
				} else {
					$response['status'] = 'error';
		 		...
		 		<?php
		 			...
		 			if($result->num_rows > 0){
		 				while($data = $result->fetch_array()) {
		 					...
		 					if($data['icon_image'] != "")
		 						$icon_image = "<img src='../$facility_img_dir/small/$data[icon_image]' style='width:30px;'>";
		 					else
		 						$icon_image = "[NO IMAGE]";
		 					...
		 					echo "<tr colspan='7'><td>No Facilities found.</td></tr>";
		 				}
		 			?>
		 		</tbody>
		 	  </table>
									...
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										<div class="input-group-prepend">
										 ...
										 <input type="file" id="icon_image" name="icon_image" class="form-control" onchange="checkFile(this)" accept="image/*" oninput="icon_image_pre.src=window.URL.createObjectURL(this.files[0])" required /><br>
										 <img id="icon_image_pre" src="" class="img-fluid" style="width:150px;" /> 
									  </div>
									</div>
								</div>
								<div class="form-group row mb-4">
	...
	<script>
		...
		$(document).on('click','.edit', function(event){
			...
			success:function(data)
			{
				...
				$('#name').val(data.name);
				 $('#icon_image_pre').attr("src", "<?php echo "../$facility_img_dir/small/"; ?>"+data.icon_image).show;
