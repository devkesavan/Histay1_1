...
if(isset($_SESSION['admin_uid']))
{
	...
	$page_title = "E.Registration";
	$sub_page_title = "E.Registration";
	$msg_title = "E.Registration";
	$THIS_TABLE = $events_registration_table;
	...
	if(isset($_POST['action']))
	{
		...
		if($_POST['action'] == 'ac_add')
		{
			...
			{
				$insert_query = "insert into $THIS_TABLE(event_id,name,gender,dob,email,mobile,created_at,created_by,status)
												values('$event_id','$name','$gender','$dob','$email','$mobile',$created_at','$created_by','$status')";
				$inst_res = $con->query($insert_query) or die(mysqli_error($con));

				if($inst_res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Event Registration Added successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Event Registration Not Added!';
				}
			} else {
				$response['status'] = 'error';
		...
		if($_POST['action'] == 'ac_edit')
		{
			...
			$update_sql = "update $THIS_TABLE set event_id='$event_id',name='$name',gender='$gender',dob='$dob',
							email='$email',mobile='$mobile',updated_at='$updated_at',updated_by='$updated_by' where id='$id'";
			...
			if($up_res === true) {
				...
				$response['status'] = 'error';
				$response['message'] = 'Event Registration are Not Updated!';
			}
			echo json_encode($response);
		}
		//viewdata fetch
		...
		{
			extract($_POST);
			$editquery = " SELECT er.*,et.hotel_id FROM $THIS_TABLE er inner join $events_table et on er.event_id=et.id WHERE er.id= '$id' ";
			$record = $con->query($editquery);
			$output = array();
			if($record->num_rows > 0)
			{
		...
		if($_POST['action'] == 'ac_change_status')
		{
			if($_POST['id'])
			{
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Event Registration Status changed successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Event Registration Status NOT changed.';
				}
				echo json_encode($response);
			}
	  }
			...
			<script>
		 	...
		 	<div class="table-responsive">
		 	  <table class="table table-striped table-hover" id="save-stage" style="width:100%;">
		 		...
		 		<tbody>
		 		<?php
		 		$select_query = "SELECT et.id,et.event_name,ht.id,ht.name as hotel_name,er.* FROM $THIS_TABLE er INNER JOIN $events_table et ON et.id=er.event_id INNER JOIN $hotels_table ht ON ht.id=et.hotel_id WHERE er.status!='E'";
		 			$result = $con->query($select_query) or die(mysqli_error($con));
		 			if($result->num_rows > 0){
		 				while($data = $result->fetch_array()) {
		 					...
		 					if($data['gender'] == "F") {
		 						$gender = '<span class="badge badge-success badge-shadow">FEMALE</span>';
		 					}else if($data['gender'] == "M"){
		 						$gender = '<span class="badge badge-primary badge-shadow">MALE</span>';
		 					}
		 					...
		 					echo "<tr colspan='7'><td>No Events Registration found.</td></tr>";
		 				}
		 			?>
		 		</tbody>
		 	  </table>
					...
					<div class="card">
						 <div class="card-header">
							...
							<div class="card-header-action">
								<a href="export_events_registration.php"><button type="button" id="export" class="btn btn-success">Export</button></a>
							</div>
							&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
							<div class="card-header-action">
									...
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										...
										<select class="form-control" name="hotel_id" id="hotel_id" required/>
    										...
    										<?php
    										$select_query = "select id,name as hotel_name from $hotels_table where status!='E'";
    	                                    $result = $con->query($select_query) or die(mysqli_error($con));
    	                                    while($data = $result->fetch_array())
    	                                    {
									...
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										...
										<select class="form-control" name="event_id" id="event_id" required/>
										    ...
										    <?php
    											$select_query = "select id,event_name from $events_table where status!='E'";
    											$result = $con->query($select_query) or die(mysqli_error($con));
    											while($data = $result->fetch_array())
    											{
    												echo "<option value='$data[id]'>$data[event_name]</option>";
    											}
                                            ?>
										</select>
									  </div>
								...
								<div class="form-group row mb-4">
									...
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										...
										<label><input type="radio" id="gender_f" name="gender" value="F" required /> FEMALE</label>&nbsp&nbsp&nbsp
										<label><input type="radio" id="gender_m" name="gender" value="M" required /> MALE</label>
										</p>
									  </div>
									</div>
								</div>
	...
	<script>
		...
		$(document).on('click','.edit', function(event){
			...
			success:function(data)
			{
				...
				$('#name').val(data.name);
				$('#gender_m').attr('checked',false);
				$('#gender_f').attr('checked',false);
				if(data.gender == "F")
