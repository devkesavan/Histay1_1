...
if(isset($_SESSION['admin_uid']))
{
	...
	if(isset($_POST['action']))
	{
		...
		if($_POST['action'] == 'ac_add')
		{
			...
			$status     = "A";
			$images	=$_FILES["icon_image"]['name'];
			
			if(file_exists("upload/".$_FILES["icon_image"]["name"]))
			{
				$store=$_FILES["icon_image"]["name"];
				$SESSION['status']="Image Alredy exists.'.$store.'";
			...
			}else
			{
				...
				if($result->num_rows == 0)
				{
					...
					$insert_query = "insert into $THIS_TABLE(name,icon_image,sort_no,created_at,created_by,status)
													values('$name','$icon_image','$sort_no','$created_at','$created_by','$status')";
									
					$inst_res = $con->query($insert_query) or die(mysqli_error($con));
					
					if($inst_res === true ){
						move_uploaded_file($_FILES["icon_image"]["tmp_name"],"upload/".$_FILES["icon_image"]["name"]);
		...
		if($_POST['action'] == 'ac_edit')
		{
			...
			$update_sql = "update $THIS_TABLE set name='$name',icon_image='$icon_image',sort_no='$sort_no',updated_at='$updated_at',updated_by='$updated_by' where id='$id'";
		 		...
		 		<?php
		 			...
		 			if($result->num_rows > 0){
		 				while($data = $result->fetch_array()) {
		 					...
		 					echo "<tr>
		 							<td>$data[name]</td>
									<td><img src="upload/'.$data['images'].'"width='100px;' height='100px;'></td>
									<td>$data[sort_no]</td>
