...
if(isset($_SESSION['admin_uid']))
{
	...
	$page_title = "Hotel Addons";
	$sub_page_title = "Hotel Addons";
	$msg_title = "Hotel Addons";
	$THIS_TABLE = $hotel_addons_table;
	...
	if(isset($_POST['action']))
	{
		...
		if($_POST['action'] == 'ac_add')
		{
			...
			$select_query = " select id from $THIS_TABLE where hotel_id='$hotel_id' and name='$name'and status!='E'";
			$result = $con->query($select_query) or die(mysqli_error($con));
			if($result->num_rows == 0)
			{
				$insert_query = "insert into $THIS_TABLE(hotel_id,name,short_description,sort_no,amount,tax_id,created_at,created_by,status)
												values('$hotel_id','$name','$short_description','$sort_no','$amount','$tax_id','$created_at','$created_by','$status')";
		...
		if($_POST['action'] == 'ac_edit')
		{
			...
			$update_sql = "update $THIS_TABLE set name='$name',short_description='$short_description',sort_no='$sort_no',amount='$amount',tax_id='$tax_id',updated_at='$updated_at',updated_by='$updated_by' where id='$id'";
