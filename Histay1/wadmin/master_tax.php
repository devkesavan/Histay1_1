...
if(isset($_SESSION['admin_uid']))
{
	...
	$page_title = "Taxes";
	$sub_page_title = "Taxes";
	$msg_title = "Taxes";
	$THIS_TABLE = $master_tax_table;
	...
	if(isset($_POST['action']))
	{
		...
		if($_POST['action'] == 'ac_add')
		{
			...
			{
				$insert_query = "insert into $THIS_TABLE(name,value,created_at,created_by,status)
												values('$name','$value','$created_at','$created_by','$status')";
				$inst_res = $con->query($insert_query) or die(mysqli_error($con));

				if($inst_res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Taxes Added successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Taxes Not Added!';
				}
			} else {
				$response['status'] = 'error';
				$response = 'Taxes Title Already exists!';
			}
		...
		if($_POST['action'] == 'ac_edit')
		{
			...
			$update_sql = "update $THIS_TABLE set name='$name',value='$value',updated_at='$updated_at',updated_by='$updated_by'where id='$id'";
			$up_res = $con->query($update_sql);
			if($up_res === true) {
				$response['status'] = 'success';
				$response['message'] = 'Taxes updated successfully!';
			} else {
				$response['status'] = 'error';
				$response['message'] = 'Taxes are Not Updated!';
			}
			echo json_encode($response);
		}
		//viewdata fetch
	 ...
	 // 							WHERE ta.status!='E' AND ta.id='$id'";
		...
		if($_POST['action'] == 'ac_change_status')
		{
			if($_POST['id'])
			{
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Taxes Status changed successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Taxes Status NOT changed.';
				}
				echo json_encode($response);
			}
	  }
		...
		if($_POST['action'] == 'ac_delete')
		{
			if($_POST['id']){
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Taxes Record deleted Successfuly...';
				} else {
					$response['status'] = 'error';
		 		...
		 		<?php
		 			...
		 			if($result->num_rows > 0){
		 				while($data = $result->fetch_array()) {
		 					...
		 					echo "<tr colspan='7'><td>No Taxes found.</td></tr>";
		 				}
		 			?>
		 		</tbody>
		 	  </table>
