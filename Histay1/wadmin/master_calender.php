...
if(isset($_SESSION['admin_uid']))
{
	...
	$page_title = "Calender";
	$sub_page_title = "Calender";
	$msg_title = "Calender";
	$THIS_TABLE = $master_calender_table;
	...
	if(isset($_POST['action']))
	{
		...
		if($_POST['action'] == 'ac_add')
		{
			...
			$select_query = " select id from $THIS_TABLE where cdate='$cdate' and status!='E' ";
			$result = $con->query($select_query) or die(mysqli_error($con));
			if($result->num_rows == 0)
			{
				$insert_query = "insert into $THIS_TABLE(cdate,title,description,holiday,created_at,created_by,status)
												values('$cdate','$title','$description','$holiday','$created_at','$created_by','$status')";
				$inst_res = $con->query($insert_query) or die(mysqli_error($con));

				if($inst_res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Calender Added successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Calender Not Added!';
				}
			} else {
				$response['status'] = 'error';
				$response = 'Calender Title Already exists!';
			}
		...
		if($_POST['action'] == 'ac_edit')
		{
			...
			$update_sql = "update $THIS_TABLE set cdate='$cdate',title='$title',description='$description',holiday='$holiday',updated_at='$updated_at',updated_by='$updated_by' where id='$id'";
			$up_res = $con->query($update_sql);
			if($up_res === true) {
				$response['status'] = 'success';
				$response['message'] = 'Calender updated successfully!';
			} else {
				$response['status'] = 'error';
				$response['message'] = 'Calender are Not Updated!';
			}
			echo json_encode($response);
		}
		//viewdata fetch
		...
		if($_POST['action'] == 'ac_change_status')
		{
			if($_POST['id'])
			{
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Calender Status changed successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Calender Status NOT changed.';
				}
				echo json_encode($response);
			}
	  }
		...
		if($_POST['action'] == 'ac_delete')
		{
			if($_POST['id']){
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Calender Record deleted Successfuly...';
				} else {
					$response['status'] = 'error';
		 		...
		 		<?php
		 			...
		 			if($result->num_rows > 0){
		 				while($data = $result->fetch_array()) {
		 					...
		 					if($data['holiday'] == "Y") {
		 						$holiday = '<span class="badge badge-success badge-shadow">YES</span>';
		 					}else if($data['holiday'] == "N"){
		 						$holiday = '<span class="badge badge-danger badge-shadow">NO</span>';
							}
		 					echo "<tr>
		 							<td> $data[cdate]</td>
		 							<td> $data[title]</td>
		 					...
		 					echo "<tr colspan='7'><td>Calender Not found.</td></tr>";
		 				}
		 			?>
		 		</tbody>
		 	  </table>
						...
						<form id="register_form" method="POST" autocomplete="off">
						    <div class="modal-body">
								<div class="form-group row mb-4">
									<label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">CDate</label>
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										<div class="input-group-prepend">
										  <div class="input-group-text"> <i class="fas fa-map-pin"></i> </div>
										</div>
										<input type="date" id="cdate" name="cdate" class="form-control" placeholder="CDate" required />
									  </div>
									</div>
								</div>
								<div class="form-group row mb-4">
								...
								<div class="form-group row mb-4">
									<label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">Holiday</label>
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										<p>
										<label><input type="radio" id="holiday_y" name="holiday" value="Y" required /> Yes</label>&nbsp&nbsp&nbsp
										<label><input type="radio" id="holiday_n" name="holiday" value="N" required /> No</label>
										</p>
									  </div>
									</div>
								</div>
	...
	<script>
		...
		$(document).on('click','.edit', function(event){
			...
			success:function(data)
			{
				...
				$('#holiday_y').attr('checked',false);
				$('#holiday_n').attr('checked',false);
				if(data.holiday == "N")
					$('#holiday_n').attr('checked',true);
				else if(data.holiday == "Y")
					$('#holiday_y').attr('checked',true);
				$('#id').val(id);
				$('#action').val('ac_edit');
				$('#save').val('Update');
				},error:function(xhr, ajaxOptions, thrownError){alert(xhr.responseText); ShowMessage("??? ?? ","fail");}
