...
if(isset($_SESSION['admin_uid']))
{
	...
	$page_title = "Master About Category";
	$sub_page_title = "Master About Category";
	$msg_title = "Master About Category";
	$THIS_TABLE =$master_about_table;
	...
	if(isset($_POST['action']))
	{
		...
		if($_POST['action'] == 'ac_add')
		{
			...
			{
				$insert_query = "insert into $THIS_TABLE(cat_id,name,description,created_at,created_by,status)
												values('$cat_id','$name','$description','$created_at','$created_by','$status')";
		...
		if($_POST['action'] == 'ac_edit')
		{
			...
			$update_sql = "update $THIS_TABLE set cat_id='$cat_id',name='$name',description='$description',updated_at='$updated_at',updated_by='$updated_by' where id='$id'";
			...
			<script>
		 	...
		 	<div class="table-responsive">
		 	  <table class="table table-striped table-hover" id="save-stage" style="width:100%;">
		 		...
		 		<tbody>
		 		<?php
		 			$select_query ="SELECT ac.id,ac.name as category_name,ma.*FROM $THIS_TABLE ma inner join  $about_category_table ac on ac.id=ma.cat_id WHERE ma.status!='E'";
		 			$result = $con->query($select_query) or die(mysqli_error($con));
		 			if($result->num_rows > 0){
									...
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										...
										<option value=''>Select Category</option>
										 <?php
											$select_query = "select id,name from $about_category_table where status!='E'";
                                        	$result = $con->query($select_query) or die(mysqli_error($con));
                                        	while($data = $result->fetch_array())
                                        	{
