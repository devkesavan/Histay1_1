...
if(isset($_SESSION['admin_uid']))
{
	...
	$page_title = "Master Events";
	$sub_page_title = "Master Events";
	$msg_title = "Master Events";
	$THIS_TABLE = $events_table;
	...
	if(isset($_POST['action']))
	{
		...
		if($_POST['action'] == 'ac_add')
		{
			...
			$select_query = " select id from $THIS_TABLE where event_name='$event_name' and hotel_id='$hotel_id' and status ='A'";
			$result = $con->query($select_query) or die(mysqli_error($con));
			if($result->num_rows == 0)
			{
				$insert_query = "insert into $THIS_TABLE(hotel_id,event_date,event_name,description,created_at,created_by,status)
												values('$hotel_id','$event_date','$event_name','$description','$created_at','$created_by','$status')";
            	...
            	{            		
            		...
            		$new_file_name = "events_".$insert_id;
				...
				if($inst_res === true ){
					...
					$response['message'] = 'Events Added successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Events Not Added!';
				}
			} else {
				$response['status'] = 'error';
				$response = 'Events Title Already exists!';
			}
		...
		if($_POST['action'] == 'ac_edit')
		{
			...
			$update_sql = "update $THIS_TABLE set hotel_id='$hotel_id',event_date='$event_date',event_name='$event_name',description='$description',image='$image',
        	...
        	{            		
        		...
        		if(empty($errors)==true)
        		{
        			...
        			$ud_query = "update $THIS_TABLE set image='$new_up_file_name' where id='$id'";
        			$con->query($ud_query) or die(mysqli_error($con));
        		}
        		else
        		{
			...
			if($up_res === true) {
				$response['status'] = 'success';
				$response['message'] = 'Events updated successfully!';
			} else {
				$response['status'] = 'error';
				$response['message'] = 'Events are Not Updated!';
			}
			echo json_encode($response);
		}
		//viewdata fetch
		...
		if($_POST['action'] == 'ac_change_status')
		{
			if($_POST['id'])
			{
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Events Status changed successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Events Status NOT changed.';
				}
				echo json_encode($response);
			}
	  }
		...
		if($_POST['action'] == 'ac_delete')
		{
			if($_POST['id']){
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Events Record deleted Successfuly...';
				} else {
					$response['status'] = 'error';
			...
			<script>
		 	...
		 	<div class="table-responsive">
		 	  <table class="table table-striped table-hover" id="save-stage" style="width:100%;">
		 		...
		 		<tbody>
		 		<?php
		 			$select_query = "SELECT ht.id,ht.name as hotel_name,et.* FROM $THIS_TABLE et
		 							INNER JOIN $hotels_table ht ON ht.id=et.hotel_id WHERE et.status!='E'";
		 			$result = $con->query($select_query) or die(mysqli_error($con));
		 			if($result->num_rows > 0){
		 				while($data = $result->fetch_array()) {
							...
							if($data['image'] != "")
		 						$image = "<img src='../$events_img_dir/small/$data[image]' style='width:100px;'>";
		 					else
		 						$image = "[NO IMAGE]";
		 					echo "<tr>
									...
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										<div class="input-group-prepend">
										 ...
										 <input type="file" id="image" name="image" class="form-control" onchange="checkFile(this)" accept="image/*" oninput="image_pre.src=window.URL.createObjectURL(this.files[0])" required /><br>
										 <img id="image_pre" src="" class="img-fluid" style="width:150px;" />
										</div>
									</div>
								</div>
	...
	<script>
		...
		$(document).on('click','.edit', function(event){
			...
			success:function(data)
			{
				...
				$('#image_pre').attr("src", "<?php echo "../$events_img_dir /small/"; ?>"+data.image).show;
				$('#id').val(id);
				$('#action').val('ac_edit');
				$('#save').val('Update');
				},error:function(xhr, ajaxOptions, thrownError){alert(xhr.responseText); ShowMessage("??? ?? ","fail");}
