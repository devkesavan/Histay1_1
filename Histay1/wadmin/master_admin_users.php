...
if(isset($_SESSION['admin_uid']))
{
	...
	$sub_page_title = "Admin Users";
	$msg_title = "Admin Users";
	$THIS_TABLE = $admin_table;
	...
	if(isset($_POST['action']))
	{
		...
		if($_POST['action'] == 'ac_add')
		{
			...
			$select_query = " select id from $THIS_TABLE where mobile='$mobile'and status!='E'";
			$result = $con->query($select_query) or die(mysqli_error($con));
			if($result->num_rows == 0)
			{
			    $gst = $id_proof = $forget_token = "";
				$insert_query = "insert into $THIS_TABLE(username,password,full_name,email,mobile,address,business_name,gst,id_proof,forget_token,user_type,created_at,created_by,status)
												values('$username','$password','$full_name','$email','$mobile','$address','$business_name','$gst','$id_proof','$forget_token','$user_type','$created_at','$created_by','$status')";
				$inst_res = $con->query($insert_query) or die(mysqli_error($con));

				if($inst_res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Admin Users Added successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Admin Users Not Added!';
				}
			} else {
				$response['status'] = 'error';
		...
		if($_POST['action'] == 'ac_edit')
		{
            ...
            $gst = $id_proof = $forget_token = "";
			$update_sql = "update $THIS_TABLE set username='$username',password='$password',full_name='$full_name',email='$email',mobile='$mobile',address='$address',business_name='$business_name',gst='$gst',id_proof='$id_proof',forget_token='$forget_token',user_type='$user_type',updated_at='$updated_at',updated_by='$updated_by' where id='$id'";
			$up_res = $con->query($update_sql);
			if($up_res === true) {
				$response['status'] = 'success';
				$response['message'] = 'Admin Users updated successfully!';
			} else {
				$response['status'] = 'error';
				$response['message'] = 'Admin Users are Not Updated!';
			}
			echo json_encode($response);
		}
		//viewdata fetch
	 ...
	 // 							WHERE ta.status!='E' AND ta.id='$id'";
		...
		if($_POST['action'] == 'ac_change_status')
		{
			if($_POST['id'])
			{
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Admin Users Status changed successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Admin Users Status NOT changed.';
				}
				echo json_encode($response);
			}
	  }
		...
		if($_POST['action'] == 'ac_delete')
		{
			if($_POST['id']){
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Admin Users Record deleted Successfuly...';
				} else {
					$response['status'] = 'error';
		 		...
		 		<?php
		 			...
		 			if($result->num_rows > 0){
		 				while($data = $result->fetch_array()) {
		 					...
		 					if($data['user_type'] == "SA") {
		 						$user_type = '<span class="badge badge-primary badge-shadow">SUPER ADMIN</span>';
		 					}else if($data['user_type'] == "AD"){
		 						$user_type = '<span class="badge badge-sucess badge-shadow">ADMIN</span>';
		 					}else if($data['user_type'] == "HA"){
		 						$user_type = '<span class="badge badge-warning badge-shadow">HOTEL ADMIN</span>';
		 					}
		 					echo "<tr>
		 							<td>$data[username]</td>
									<td>$data[mobile]</td>
		 					...
		 					echo "<tr colspan='7'><td>No Admin Users found.</td></tr>";
		 				}
		 			?>
		 		</tbody>
		 	  </table>
					  ...
					  <div class="modal-dialog modal-dialog-centered" role="document">
						...
						<form id="register_form" method="POST" autocomplete="off">
						    <div class="modal-body">
								...
								<div class="form-group row mb-4">
									<label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">Email Id</label>
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										<div class="input-group-prepend">
										  <div class="input-group-text"> <i class="fas fa-map-pin"></i> </div>
								...
								<div class="form-group row mb-4">
									...
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										 <p>
										 <label><input type="radio" id="user_type_sa" name="user_type" value="SA" required /> SUPER ADMIN</label>&nbsp&nbsp&nbsp
										 <label><input type="radio" id="user_type_ad" name="user_type" value="AD" required /> ADMIN</label>
										 <label><input type="radio" id="user_type_ha" name="user_type" value="HA" required /> HOTEL ADMIN</label>
										 </p>
									  </div>
									</div>
								</div>
	...
	<script>
		...
		$(document).on('click','.edit', function(event){
			...
			success:function(data)
			{
				...
				$('#forget_token').val(data.forget_token);
				$('#user_type_sa').attr('checked',false);
				$('#user_type_ad').attr('checked',false);
				$('#user_type_em').attr('checked',false);
				if(data.user_type == "SA")
					$('#user_type_sa').attr('checked',true);
				else if(data.user_type == "AD")
					$('#user_type_ad').attr('checked',true);
				else if(data.user_type == "HA")
					$('#user_type_ha').attr('checked',true);
				$('#id').val(id);
				$('#action').val('ac_edit');
				$('#save').val('Update');
				},error:function(xhr, ajaxOptions, thrownError){alert(xhr.responseText); ShowMessage("??? ?? ","fail");}
