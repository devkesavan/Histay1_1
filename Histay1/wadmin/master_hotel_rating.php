...
if(isset($_SESSION['admin_uid']))
{
	...
	$page_title = "Hotel Rating";
	$sub_page_title = "Hotel Rating";
	$msg_title = "Hotel Rating";
	$THIS_TABLE = $hotel_rating_tables;
	...
	if(isset($_POST['action']))
	{
		...
		if($_POST['action'] == 'ac_add')
		{
			...
			{
				$insert_query = "insert into $THIS_TABLE(hotel_id,name,email,comment,rating,sort_no,created_at,created_by,status)
												values('$hotel_id','$name','$email','$comment','$rating','$sort_no','$created_at','$created_by','$status')";
				$inst_res = $con->query($insert_query) or die(mysqli_error($con));

				if($inst_res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Hotel Class Added successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Hotel Class Not Added!';
				}
			} else {
				$response['status'] = 'error';
				$response = 'Hotel Class Title Already exists!';
			}
		...
		if($_POST['action'] == 'ac_edit')
		{
			...
			$update_sql = "update $THIS_TABLE set hotel_id='$hotel_id',name='$name',email='$email',comment='$comment',rating='$rating',sort_no='$sort_no',updated_at='$updated_at',updated_by='$updated_by' where id='$id'";
			$up_res = $con->query($update_sql);
			if($up_res === true) {
				$response['status'] = 'success';
				$response['message'] = 'Hotel Rating updated successfully!';
			} else {
				$response['status'] = 'error';
				$response['message'] = 'Hotel Rating are Not Updated!';
			}
			echo json_encode($response);
		}
		//viewdata fetch
		 	...
		 	<div class="table-responsive">
		 	  <table class="table table-striped table-hover" id="save-stage" style="width:100%;">
		 		...
		 		<?php
		 			...
		 			{
		 				$select_query = "SELECT ht.id,ht.name as hotel_name,hr.* FROM $THIS_TABLE hr 
		 							INNER JOIN $hotels_table ht ON ht.id=hr.hotel_id WHERE hr.hotel_id='$_SESSION[admin_uid]'";
		 			}
