...
if(isset($_SESSION['admin_uid']))
{
	...
	$page_title = "Country Master";
	$sub_page_title = "Plan Master";
	$msg_title = "Country Master";
	$THIS_TABLE = $master_country_table;
	...
	if(isset($_POST['action']))
	{
		...
		if($_POST['action'] == 'ac_add')
		{
			...
			$select_query = " select id from $THIS_TABLE where country_name='$country_name' and status!='E' ";
			$result = $con->query($select_query) or die(mysqli_error($con));
			if($result->num_rows == 0)
			{
				$insert_query = "insert into $THIS_TABLE(country_code,country_name,created_at,created_by,status)
												values('$country_code','$country_name','$created_at','$created_by','$status')";
				...
				$response['status'] = 'error';
				$response = 'Country Title Already exists!';
			}
		...
		if($_POST['action'] == 'ac_edit')
		{
			...
			$update_sql = "update $THIS_TABLE set country_code='$country_code',country_name='$country_name',updated_at='$updated_at',updated_by='$updated_by' where id='$id'";
			...
			if($up_res === true) {
				...
				$response['status'] = 'error';
				$response['message'] = 'Country are Not Updated!';
			}
			echo json_encode($response);
		}
		//viewdata fetch
		...
		if($_POST['action'] == 'ac_change_status')
		{
			if($_POST['id'])
			{
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Country Status changed successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Country  Status NOT changed.';
				}
				echo json_encode($response);
			}
	  }
		...
		if($_POST['action'] == 'ac_delete')
		{
			if($_POST['id']){
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Country deleted Successfuly...';
				} else {
					$response['status'] = 'error';
						...
						<form id="register_form" method="POST" autocomplete="off">
						    <div class="modal-body">
								<div class="form-group row mb-4">
									<label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">Country Code</label>
