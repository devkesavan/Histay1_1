...
if(isset($_SESSION['admin_uid']))
{
	...
	$sub_page_title = "Department";
	$msg_title = "Department";
	$THIS_TABLE = $master_department_table;
	...
	if(isset($_POST['action']))
	{
		...
		if($_POST['action'] == 'ac_add')
		{
			...
			{
				$insert_query = "insert into $THIS_TABLE(name,department,created_at,created_by,status)
												values('$name','$department','$created_at','$created_by','$status')";
				...
				$response['status'] = 'error';
				$response = 'Department Title Already exists!';
			}
		...
		if($_POST['action'] == 'ac_edit')
		{
			...
			$update_sql = "update $THIS_TABLE set name='$name',department='$department',updated_at='$updated_at',updated_by='$updated_by' where id='$id'";
			...
			if($up_res === true) {
				...
				$response['status'] = 'error';
				$response['message'] = 'Department are Not Updated!';
			}
			echo json_encode($response);
		}
		//viewdata fetch
		...
		if($_POST['action'] == 'ac_change_status')
		{
			if($_POST['id'])
			{
				...
				if($res === true ){
					...
					$response['status'] = 'error';
					$response['message'] = 'Department Status NOT changed.';
				}
				echo json_encode($response);
			}
	  }
		...
		if($_POST['action'] == 'ac_delete')
		{
			if($_POST['id']){
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Department Record deleted Successfuly...';
				} else {
					$response['status'] = 'error';
	...
	<script>
		$(document).ready(function (){
			...
			$('#usermodal').click(function(){
				...
				$('.modal-title').html('<i class="fas fa-list-alt"></i> Add');
				$('#save').val('Save');
			});
