...
if(isset($_SESSION['admin_uid']))
{
	...
	if(isset($_POST['action']))
	{
		...
		if($_POST['action'] == 'ac_add')
		{
			...
			{
				$insert_query = "insert into $THIS_TABLE(name,email,gender,language,country,gems,star,coins,created_at,created_by,status)
											values('$name','$email','$language','$country',''$gems',$star,$coins','$created_at','$created_by','$status')";
		...
		if($_POST['action'] == 'ac_edit')
		{
			...
			$update_sql = "update $THIS_TABLE set name='$name',email='$email',language='$language',country='$country',
			...
			<script>
		 	...
		 	<div class="table-responsive">
		 	  <table class="table table-striped table-hover" id="save-stage" style="width:100%;">
		 		...
		 		<tbody>
		 		<?php
		 			$select_query = "SELECT mt.*,ct.name as country,lt.name as language
									FROM $THIS_TABLE mt inner join  $country_table ct on mt.country_id=ct.id
									INNER JOIN $language_table lt ON mt.language_id=lt.id WHERE mt.status!='E'";
		 			$result = $con->query($select_query) or die(mysqli_error($con));
					if($result->num_rows > 0)
					{
						...
						{
							if($data['gender'] == "M")
								$gender = '<span class="badge badge-success badge-shadow">Male</span>';
							elseif($data['gender'] == "F")
								$gender = '<span class="badge badge-danger badge-shadow">Female</span>';
							elseif($data['gender'] == "O")
								$gender = '<span class="badge badge-warning badge-shadow">Others</span>';

							if($data['vip_membership'] == "Y")
								$vip_membership = '<span class="badge badge-success badge-shadow">YES</span>';
							elseif($data['vip_membership'] == "N")
								$vip_membership = '<span class="badge badge-danger badge-shadow">NO</span>';

							if($data['profile_img'] != "")
								$profile_img = "<img src='$profile_img_dir/$data[profile_img]' style='width:50px;'>";
							else
								$profile_img = "<img src='$profile_img_dir/default.png' style='width:50px;'>";

		 					echo "<tr>
		 							<td>$profile_img <button type='button' class='btn btn-primary btn-sm remove_dp' data-id='$data[id]'><i class='fas fa-sync-alt'></i></button><br>$data[name]</td>
		 							<td>$data[email]</td>
									<td>$gender</td>
									<td>$data[language]</td>
									<td>$data[country]</td>
									<td>$data[gems]</td>
									<td>$vip_membership</td>
									<td>$data[coins]</td>
								...
								<div class="form-group row mb-4">
									...
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										...
										<input type="numher" id="gems" name="gems" class="form-control" required />
										</div>
									</div>
								</div>
								<div class="form-group row mb-4">
									...
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										...
										<input type="radio" id="star" name="star" class="form-control" required />
										</div>
									</div>
								</div>
								<div class="form-group row mb-4">
									<label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">Coins</label>
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										<div class="input-group-prepend">
											<div class="input-group-text"> <i class="fas fa-map-marker-alt"></i> </div>
										</div>
										<input type="numher" id="coins" name="coins" class="form-control" required />
										</div>
									</div>
								</div>
