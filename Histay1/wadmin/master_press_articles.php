...
if(isset($_SESSION['admin_uid']))
{
	...
	$page_title = "Master Press Articles";
	$sub_page_title = "Master Press Articles";
	$msg_title = "Master Press Articles";
	$THIS_TABLE = $master_press_articles;
	...
	if(isset($_POST['action']))
	{
		...
		if($_POST['action'] == 'ac_add')
		{
			...
			{
				$insert_query = "insert into $THIS_TABLE(title,short_description,description,image,created_at,created_by,status)VALUES('$title','$short_description','$description','$image','$created_at','$created_by','$status')";
				$inst_res = $con->query($insert_query) or die(mysqli_error($con));

				if($inst_res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Articles Added successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Articles Not Added!';
				}
			} else {
				$response['status'] = 'error';
				$response = 'Articles Already exists!';
			}
		...
		if($_POST['action'] == 'ac_edit')
		{
			...
			$update_sql = "update $THIS_TABLE set title='$title',short_description='$short_description',description='$description',image='$image',
							updated_at='$updated_at',updated_by='$updated_by' where id='$id'";
			$up_res = $con->query($update_sql);
			if($up_res === true) {
				$response['status'] = 'success';
				$response['message'] = 'Articles updated successfully!';
			} else {
				$response['status'] = 'error';
				$response['message'] = 'Articles are Not Updated!';
			}
			echo json_encode($response);
		}
		//viewdata fetch
		...
		if($_POST['action'] == 'ac_change_status')
		{
			if($_POST['id'])
			{
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Articles Status changed successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Articles  Status NOT changed.';
				}
				echo json_encode($response);
			}
	  }
		...
		if($_POST['action'] == 'ac_delete')
		{
			if($_POST['id']){
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Articles Record deleted Successfuly...';
				} else {
					$response['status'] = 'error';
								...
								<div class="form-group row mb-4">
									...
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										...
										<textarea class='teditor' name="description" class="form-control" required ></textarea>
									  </div>
									</div>
								</div>
								<div class="form-group row mb-4">
