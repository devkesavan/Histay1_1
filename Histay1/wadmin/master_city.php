...
if(isset($_SESSION['admin_uid']))
{
	...
	$page_title = "Master City";
	$sub_page_title = "Master City";
	$msg_title = "Master";
	$THIS_TABLE = $master_city_table;
	...
	if(isset($_POST['action']))
	{
		...
		if($_POST['action'] == 'ac_add')
		{
			...
			$select_query = " select id from $THIS_TABLE where city='$city' and status!='E' ";
			$result = $con->query($select_query) or die(mysqli_error($con));
			if($result->num_rows == 0)
			{
				$insert_query = "insert into $THIS_TABLE(state_id,city,created_at,created_by,status)VALUES('$state_id','$city','$created_at','$created_by','$status')";
				...
				if($inst_res === true ){
					...
					$response['message'] = 'City Not Added!';
				}
			} else {
				$response['status'] = 'error';
		...
		if($_POST['action'] == 'ac_edit')
		{
			...
			$update_sql = "update $THIS_TABLE set state_id='$state_id',city='$city',
			...
			if($up_res === true) {
				...
				$response['status'] = 'error';
				$response['message'] = 'city are Not Updated!';
			}
			echo json_encode($response);
		}
		//viewdata fetch
		...
		if($_POST['action'] == 'ac_change_status')
		{
			if($_POST['id'])
			{
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'city Status changed successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'city  Status NOT changed.';
				}
				echo json_encode($response);
			}
	  }
		...
		if($_POST['action'] == 'ac_delete')
		{
			if($_POST['id']){
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'city Record deleted Successfuly...';
				} else {
					$response['status'] = 'error';
			...
			<script>
		 	...
		 	<div class="table-responsive">
		 	  <table class="table table-striped table-hover" id="save-stage" style="width:100%;">
		 		...
		 		<tbody>
		 		<?php
		 			$select_query = "SELECT ct.*,sm.state from $THIS_TABLE ct inner join $master_state_table sm on ct.state_id=sm.id WHERE ct.status!='E' order by ct.id asc";
		 			$result = $con->query($select_query) or die(mysqli_error($con));
		 			if($result->num_rows > 0){
		 				while($data = $result->fetch_array()) {
		 					...
		 					echo "<tr colspan='7'><td>No Attributes found.</td></tr>";
		 				}
		 			?>
		 		</tbody>
		 	  </table>
	...
	<script>
		$(document).ready(function (){
			...
			$('#usermodal').click(function(){
				...
				$('.modal-title').html('<i class="fas fa-list-alt"></i> Add City Master');
				$('#save').val('Add City');
