...
if(isset($_SESSION['admin_uid']))
{
	...
	$sub_page_title = "Master Gallery ";
	$msg_title = "Master Gallery";
	$THIS_TABLE =$master_gallery_table;
	...
	if(isset($_POST['action']))
	{
		...
		if($_POST['action'] == 'ac_add')
		{
			...
			{
				$insert_query = "insert into $THIS_TABLE(gallery_cat_id,title,image,sort_no,created_at,created_by,status)
												values('$gallery_cat_id','$name','$image','$sort_no','$created_at','$created_by','$status')";
				...
				if($inst_res === true ){
					...
					$response['message'] = 'Gallery Not Added!';
				}
			} else {
				$response['status'] = 'error';
				$response = 'Gallery Title Already exists!';
			}
		...
		if($_POST['action'] == 'ac_edit')
		{
			...
			$update_sql = "update $THIS_TABLE set category_id='$category_id',title='$title',image='$image',sort_no='$sort_no',updated_at='$updated_at',updated_by='$updated_by' where id='$id'";
			...
			if($up_res === true) {
				...
				$response['status'] = 'error';
				$response['message'] = 'Gallery are Not Updated!';
			}
			echo json_encode($response);
		}
		//viewdata fetch
		...
		if($_POST['action'] == 'ac_change_status')
		{
			if($_POST['id'])
			{
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Gallery Status changed successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Gallery Status NOT changed.';
				}
				echo json_encode($response);
			}
	  }
		...
		if($_POST['action'] == 'ac_delete')
		{
			if($_POST['id']){
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Gallery Record deleted Successfuly...';
				} else {
					$response['status'] = 'error';
			...
			<script>
		 	...
		 	<div class="table-responsive">
		 	  <table class="table table-striped table-hover" id="save-stage" style="width:100%;">
		 		...
		 		<tbody>
		 		<?php
		 			$select_query ="SELECT gc.id,gc.name as category_name,mg.* FROM $THIS_TABLE mg INNER JOIN $master_gallery_category_table gc ON gc.id=mg.gallery_cat_id WHERE mg.status!='E'";
		 			$result = $con->query($select_query) or die(mysqli_error($con));
		 			if($result->num_rows > 0){
									...
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										...
										<option value=''>Select Category</option>
										 <?php
											$select_query = "select id,name from $master_gallery_category_table where status!='E'";
                                        	$result = $con->query($select_query) or die(mysqli_error($con));
                                        	while($data = $result->fetch_array())
                                        	{
								...
								<div class="form-group row mb-4">
									...
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										...
										<textarea type="text" id="image" name="image" class="form-control" required ></textarea> 
									  </div>
									</div>
								</div>
								<div class="form-group row mb-4">
	...
	<script>
		...
		$(document).on('click','.edit', function(event){
			...
			success:function(data)
			{
				...
				$('#gallery_cat_id').val(data.gallery_cat_id);
