...
if(isset($_SESSION['admin_uid']))
{
	...
	$sub_page_title = "Notice Board";
	$msg_title = "Notice Board";
	$THIS_TABLE = $master_notice_board_table;
	...
	if(isset($_POST['action']))
	{
		...
		if($_POST['action'] == 'ac_add')
		{
			...
			{
				$insert_query = "insert into $THIS_TABLE(title,description,created_at,created_by,status)
												values('$title','$description','$created_at','$created_by','$status')";
				$inst_res = $con->query($insert_query) or die(mysqli_error($con));

				if($inst_res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Notice Board Added successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Notice Board Not Added!';
				}
			} else {
				$response['status'] = 'error';
				$response = 'Notice Board Title Already exists!';
			}
		...
		if($_POST['action'] == 'ac_edit')
		{
			...
			$update_sql = "update $THIS_TABLE set title='$title',description='$description',updated_at='$updated_at',updated_by='$updated_by' where id='$id'";
			$up_res = $con->query($update_sql);
			if($up_res === true) {
				$response['status'] = 'success';
				$response['message'] = 'Notice Board updated successfully!';
			} else {
				$response['status'] = 'error';
				$response['message'] = 'Notice Board are Not Updated!';
			}
			echo json_encode($response);
		}
		//viewdata fetch
		...
		if($_POST['action'] == 'ac_change_status')
		{
			if($_POST['id'])
			{
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Notice Board Status changed successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Notice Board Status NOT changed.';
				}
				echo json_encode($response);
			}
	  }
		...
		if($_POST['action'] == 'ac_delete')
		{
			if($_POST['id']){
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Notice Board Record deleted Successfuly...';
				} else {
					$response['status'] = 'error';
		 		...
		 		<?php
		 			...
		 			if($result->num_rows > 0){
		 				while($data = $result->fetch_array()) {
		 					...
		 					echo "<tr colspan='7'><td>Notice Board Not found.</td></tr>";
		 				}
		 			?>
		 		</tbody>
		 	  </table>
