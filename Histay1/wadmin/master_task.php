...
if(isset($_SESSION['admin_uid']))
{
	...
	$page_title = "Master Tasks";
	$sub_page_title = "Master Tasks";
	$msg_title = "Master Tasks";
	$THIS_TABLE = $master_task_table;
	...
	if(isset($_POST['action']))
	{
		...
		if($_POST['action'] == 'ac_add')
		{
			...
			$select_query = " select id from $THIS_TABLE where task_name='$task_name' and status!='E' ";
			$result = $con->query($select_query) or die(mysqli_error($con));
			if($result->num_rows == 0)
			{
				$insert_query = "insert into $THIS_TABLE(project_id,task_name,description,task_status,created_at,created_by,status)
												values('$project_id','$task_name','$description','$task_status','$created_at','$created_by','$status')";
				...
				$response['status'] = 'error';
				$response = 'Task Title Already exists!';
			}
		...
		if($_POST['action'] == 'ac_edit')
		{
			...
			$update_sql = "update $THIS_TABLE set project_id='$project_id',task_name='$task_name',description='$description',task_status='$task_status',updated_at='$updated_at',updated_by='$updated_by' where id='$id'";
			...
			if($up_res === true) {
				...
				$response['status'] = 'error';
				$response['message'] = 'Task are Not Updated!';
			}
			echo json_encode($response);
		}
		//viewdata fetch
		...
		if($_POST['action'] == 'ac_change_status')
		{
			if($_POST['id'])
			{
				...
				if($res === true ){
					...
					$response['status'] = 'error';
					$response['message'] = 'Task Status NOT changed.';
				}
				echo json_encode($response);
			}
	  }
		...
		if($_POST['action'] == 'ac_delete')
		{
			if($_POST['id']){
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Task Record deleted Successfuly...';
				} else {
					$response['status'] = 'error';
			...
			<script>
		 	...
		 	<div class="table-responsive">
		 	  <table class="table table-striped table-hover" id="save-stage" style="width:100%;">
		 		...
		 		<tbody>
		 		<?php
		 			$select_query = "SELECT pr.id,pr.name as project_name,ta.* FROM $THIS_TABLE ta INNER JOIN $master_projects_table pr ON pr.id=ta.project_id WHERE ta.status!='E'";
		 			$result = $con->query($select_query) or die(mysqli_error($con));
		 			if($result->num_rows > 0){
		 				while($data = $result->fetch_array()) {
		 					...
		 					echo "<tr colspan='7'><td>No Task found.</td></tr>";
		 				}
		 			?>
		 		</tbody>
		 	  </table>
					  ...
					  <div class="modal-dialog modal-dialog-centered" role="document">
						...
						<form id="register_form" method="POST" autocomplete="off">
						    <div class="modal-body">
								...
								<div class="form-group row mb-4">
									<label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">Task Status</label>
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										<p>
	...
	<script>
		$(document).ready(function (){
			...
			$('#usermodal').click(function(){
				...
				$('.modal-title').html('<i class="fas fa-list-alt"></i> Add Task');
				$('#save').val('Save');
			});
		...
		$(document).on('click','.view', function(event){
			...
			$.ajax({
				...
				success:function(data)
				{
				   ...
				   $('#task_name').val(data.task_name);
				   $('#description').val(data.description);
				   $('#featured_cat_y').attr('checked',false);
				   $('#featured_cat_n').attr('checked',false);
				   if(data.task_status == "YTS")
				   		$('#task_status_y').attr('checked',true);
				   else if(data.task_status == "WIP")
						$('#task_status_w').attr('checked',true);
