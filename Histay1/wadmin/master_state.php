...
if(isset($_SESSION['admin_uid']))
{
	...
	$sub_page_title = "State Master";
	$msg_title = "State Master";
	$THIS_TABLE = $master_state_table;
	...
	if(isset($_POST['action']))
	{
		...
		if($_POST['action'] == 'ac_add')
		{
			...
			$select_query = " select id from $THIS_TABLE where state='$state' and status!='E' ";
			$result = $con->query($select_query) or die(mysqli_error($con));
			if($result->num_rows == 0)
			{
				$insert_query = "insert into $THIS_TABLE(country_id,state,created_at,created_by,status)
												values('$country_id','$state','$created_at','$created_by','$status')";
				...
				if($inst_res === true ){
					...
					$response['message'] = 'State Not Added!';
				}
			} else {
				$response['status'] = 'error';
				$response = 'State Title Already exists!';
			}
		...
		if($_POST['action'] == 'ac_edit')
		{
			...
			$update_sql = "update $THIS_TABLE set country_id='$country_id',state='$state',updated_at='$updated_at',updated_by='$updated_by' where id='$id'";
			...
			if($up_res === true) {
				...
				$response['status'] = 'error';
				$response['message'] = 'State are Not Updated!';
			}
			echo json_encode($response);
		}
		//viewdata fetch
		...
		if($_POST['action'] == 'ac_change_status')
		{
			if($_POST['id'])
			{
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'State Status changed successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'State Status NOT changed.';
				}
				echo json_encode($response);
			}
	  }
		...
		if($_POST['action'] == 'ac_delete')
		{
			if($_POST['id']){
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'State deleted Successfuly...';
				} else {
					$response['status'] = 'error';
			...
			<script>
		 	...
		 	<div class="table-responsive">
		 	  <table class="table table-striped table-hover" id="save-stage" style="width:100%;">
		 		...
		 		<tbody>
		 		<?php
					$select_query="SELECT ct.country_name,st.* FROM $master_state_table st INNER JOIN $master_country_table ct ON st.country_id=ct.id  where st.status!='E' order by st.id asc";
		 			$result = $con->query($select_query) or die(mysqli_error($con));
		 			if($result->num_rows > 0){
									...
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										...
										<select name="country_id" id="country_id" class="form-control" required>
											<?php
											 $select_query = " select id,country_name from $master_country_table  where status!='E' ";
											 $result = $con->query($select_query) or die(mysqli_error($con));
											 while($data = $result->fetch_array())
											 {
	...
	<script>
		$(document).ready(function (){
			...
			$('#usermodal').click(function(){
				...
				$('.modal-title').html('<i class="fas fa-list-alt"></i> Add State');
				$('#save').val('State Add');
