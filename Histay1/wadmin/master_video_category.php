...
if(isset($_SESSION['admin_uid']))
{
	...
	$page_title = "Master Video Category";
	$sub_page_title = "Master Video Category";
	$msg_title = "Master Video Category";
	$THIS_TABLE =$master_video_category_table;
	...
	if(isset($_POST['action']))
	{
		...
		if($_POST['action'] == 'ac_add')
		{
			...
			{
				$insert_query = "insert into $THIS_TABLE(name,description,created_at,created_by,status)
												values('$name','$description','$created_at','$created_by','$status')";
				$inst_res = $con->query($insert_query) or die(mysqli_error($con));

				if($inst_res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Video Category Added successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Video Category Not Added!';
				}
			} else {
				$response['status'] = 'error';
				$response = 'Video Category Title Already exists!';
			}
		...
		if($_POST['action'] == 'ac_edit')
		{
			...
			$update_sql = "update $THIS_TABLE set name='$name',description='$description',updated_at='$updated_at',updated_by='$updated_by' where id='$id'";
			$up_res = $con->query($update_sql);
			if($up_res === true) {
				$response['status'] = 'success';
				$response['message'] = 'Video Category updated successfully!';
			} else {
				$response['status'] = 'error';
				$response['message'] = 'Video Category are Not Updated!';
			}
			echo json_encode($response);
		}
		//viewdata fetch
		...
		if($_POST['action'] == 'ac_change_status')
		{
			if($_POST['id'])
			{
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Video Category Status changed successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Video Category Status NOT changed.';
				}
				echo json_encode($response);
			}
	  }
		...
		if($_POST['action'] == 'ac_delete')
		{
			if($_POST['id']){
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Video Category Record deleted Successfuly...';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Video to delete this Record..';
				}
				echo json_encode($response);
			}
		}
		 		...
		 		<?php
		 			...
		 			if($result->num_rows > 0){
		 				while($data = $result->fetch_array()) {
		 					...
		 					echo "<tr colspan='7'><td>No Gallery found.</td></tr>";
		 				}
		 			?>
		 		</tbody>
		 	  </table>
