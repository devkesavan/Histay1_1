...
if(isset($_SESSION['admin_uid']))
{
	...
	$page_title = "G.Category";
	$sub_page_title = "Gallery Category";
	$msg_title = "Gallery Category";
	$THIS_TABLE =$master_gallery_category_table;
	...
	if(isset($_POST['action']))
	{
		...
		if($_POST['action'] == 'ac_add')
		{
			...
			if($result->num_rows == 0)
			{
				...
				if($inst_res === true ){
					...
					$response['message'] = 'Gallery Category Added successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Gallery Category Not Added!';
				}
			} else {
				$response['status'] = 'error';
				$response = 'Gallery Category Title Already exists!';
			}
		...
		if($_POST['action'] == 'ac_edit')
		{
			...
			if($up_res === true) {
				$response['status'] = 'success';
				$response['message'] = 'Gallery Category updated successfully!';
			} else {
				$response['status'] = 'error';
				$response['message'] = 'Gallery Category are Not Updated!';
			}
			echo json_encode($response);
		}
		//viewdata fetch
		...
		if($_POST['action'] == 'ac_change_status')
		{
			if($_POST['id'])
			{
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Gallery Category Status changed successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Gallery Category Status NOT changed.';
				}
				echo json_encode($response);
			}
	  }
		...
		if($_POST['action'] == 'ac_delete')
		{
			if($_POST['id']){
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Gallery Category Record deleted Successfuly...';
				} else {
					$response['status'] = 'error';
