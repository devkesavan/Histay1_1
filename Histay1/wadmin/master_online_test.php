...
if(isset($_SESSION['admin_uid']))
{
	...
	$THIS_TABLE = $master_online_test_table;
	...
	if(isset($_POST['action']))
	{
		...
		if($_POST['action'] == 'ac_add')
		{
			...
			{
				$insert_query = "insert into $THIS_TABLE(name,description,questions,total_marks,pass_marks,total_minutes,created_at,created_by,status)VALUES('$name','$description','$questions','$total_marks','$pass_marks','$total_minutes','$created_at','$created_by','$status')";
				$inst_res = $con->query($insert_query) or die(mysqli_error($con));

				if($inst_res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Online Test Added successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Online Test Not Added!';
				}
			} else {
				$response['status'] = 'error';
				$response = 'Online Test Already exists!';
			}
		...
		if($_POST['action'] == 'ac_edit')
		{
			...
			$update_sql = "update $THIS_TABLE set name='$name',description='$description',questions='$questions',total_marks='$total_marks',pass_marks='$pass_marks',total_minutes='$total_minutes',updated_at='$updated_at',updated_by='$updated_by' where id='$id'";
			$up_res = $con->query($update_sql);
			if($up_res === true) {
				$response['status'] = 'success';
				$response['message'] = 'Online Test updated successfully!';
			} else {
				$response['status'] = 'error';
				$response['message'] = 'Online Test are Not Updated!';
			}
			echo json_encode($response);
		}
		//viewdata fetch
		...
		if($_POST['action'] == 'ac_change_status')
		{
			if($_POST['id'])
			{
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Online Test Status changed successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Online Test  Status NOT changed.';
				}
				echo json_encode($response);
			}
	  }
		...
		if($_POST['action'] == 'ac_delete')
		{
			if($_POST['id']){
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Online Test Record deleted Successfuly...';
				} else {
					$response['status'] = 'error';
		 			...
		 			if($result->num_rows > 0){
		 				while($data = $result->fetch_array()) {
							...
							echo "<tr>
		 					...
		 					echo "<tr colspan='7'><td>No Online Test found.</td></tr>";
		 				}
		 			?>
		 		</tbody>
		 	  </table>
					  ...
					  <div class="modal-dialog modal-dialog-centered" role="document">
						...
						<form id="register_form" method="POST" autocomplete="off">
						    <div class="modal-body">
								...
								<div class="form-group row mb-4">
									...
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										...
										<textarea rows="4" type="text" id="description" name="description" class="form-control"></textarea> 
									  </div>
									</div>
								</div>
								<div class="form-group row mb-4">
								<label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">Questions</label>
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										<div class="input-group-prepend">
										  <div class="input-group-text"><i class="fas fa-pencil-alt"></i></div>
								...
								<div class="form-group row mb-4">
									<label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">Pass Marks</label>
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										<div class="input-group-prepend">
										  <div class="input-group-text"><i class="fas fa-pencil-alt"></i></div>
										</div>
										<input type="number" id="pass_marks" name="pass_marks" class="form-control" required />
									  </div>
									</div>
								</div>
								<div class="form-group row mb-4">
									<label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">Total Minutes</label>
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										<div class="input-group-prepend">
										  <div class="input-group-text"> <i class="fas fa-pencil-alt"></i> </div>
										</div>
										<input type="number" id="total_minutes" name="total_minutes" class="form-control" required />
									  </div>
									</div>
								</div>
