...
if(isset($_SESSION['admin_uid']))
{
	...
	$page_title = "Master Videos";
	$sub_page_title = "Master Videos ";
	$msg_title = "Master Videos";
	$THIS_TABLE =$master_videos_table;
	...
	if(isset($_POST['action']))
	{
		...
		if($_POST['action'] == 'ac_add')
		{
			...
			{
				$insert_query = "insert into $THIS_TABLE(category_id,title,youtube_url,created_at,created_by,status)
												values('$category_id','$title','$youtube_url','$created_at','$created_by','$status')";
				...
				if($inst_res === true ){
					...
					$response['message'] = 'Video Not Added!';
				}
			} else {
				$response['status'] = 'error';
				$response = 'Video Title Already exists!';
			}
		...
		if($_POST['action'] == 'ac_edit')
		{
			...
			$update_sql = "update $THIS_TABLE set category_id='$category_id',title='$title',youtube_url='$youtube_url',updated_at='$updated_at',updated_by='$updated_by' where id='$id'";
			...
			if($up_res === true) {
				...
				$response['status'] = 'error';
				$response['message'] = 'Video are Not Updated!';
			}
			echo json_encode($response);
		}
		//viewdata fetch
		...
		if($_POST['action'] == 'ac_change_status')
		{
			if($_POST['id'])
			{
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Video Status changed successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Video Status NOT changed.';
				}
				echo json_encode($response);
			}
	  }
		...
		if($_POST['action'] == 'ac_delete')
		{
			if($_POST['id']){
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Video Record deleted Successfuly...';
				} else {
					$response['status'] = 'error';
			...
			<script>
		 	...
		 	<div class="table-responsive">
		 	  <table class="table table-striped table-hover" id="save-stage" style="width:100%;">
		 		...
		 		<tbody>
		 		<?php
		 			$select_query ="SELECT mc.id,mc.name as category_name,mv.* FROM $THIS_TABLE mv INNER JOIN $master_video_category_table mc 
		 							ON mc.id=mv.category_id WHERE mv.status!='E'";
		 			$result = $con->query($select_query) or die(mysqli_error($con));
		 			if($result->num_rows > 0){
		 				while($data = $result->fetch_array()) {
		 					if($data['status'] == "A") {
		 						...
		 						$youtube_url = "<a href='$data[youtube_url]' target='_blank'><span class='badge badge-success badge-shadow'>LINK</span></a>";
		 					echo "<tr>
		 							<td style='text-align:center'> $data[category_name]</td>
		 							<td style='text-align:center'> $data[title]</td>
									<td style='text-align:center'> $youtube_url</td>
		 					...
		 					echo "<tr colspan='7'><td>No Videos found.</td></tr>";
		 				}
		 			?>
		 		</tbody>
		 	  </table>
									...
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										...
										<option value=''>Select Category</option>
										 <?php
											$select_query = "select id,name from $master_video_category_table where status!='E'";
                                        	$result = $con->query($select_query) or die(mysqli_error($con));
                                        	while($data = $result->fetch_array())
                                        	{
								...
								<div class="form-group row mb-4">
									...
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										...
										<textarea type="text" id="youtube_url" name="youtube_url" class="form-control" required ></textarea> 
									  </div>
									</div>
								</div>
