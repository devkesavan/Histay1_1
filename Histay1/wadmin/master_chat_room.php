...
if(isset($_SESSION['admin_uid']))
{
	...
	$sub_page_title = "Chat Room";
	$msg_title = "Chat Room";
	$THIS_TABLE = $master_chat_room_table;
	...
	if(isset($_POST['action']))
	{
		...
		if($_POST['action'] == 'ac_add')
		{
			...
			{
				$insert_query = "insert into $THIS_TABLE(from_id,to_id,message,created_at,created_by,status)
												values('$emp_id','$task_id','$message','$created_at','$created_by','$status')";
				$inst_res = $con->query($insert_query) or die(mysqli_error($con));

				if($inst_res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Chat Room Added successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Chat Room Not Added!';
				}
			} else {
				$response['status'] = 'error';
				$response = 'Chat Room Title Already exists!';
			}
		...
		if($_POST['action'] == 'ac_edit')
		{
			...
			$update_sql = "update $THIS_TABLE set from_id='$from_id',to_id='$to_id',message='$message',updated_at='$updated_at',updated_by='$updated_by' where id='$id'";
			$up_res = $con->query($update_sql);
			if($up_res === true) {
				$response['status'] = 'success';
				$response['message'] = 'Chat Room updated successfully!';
			} else {
				$response['status'] = 'error';
				$response['message'] = 'Chat Room are Not Updated!';
			}
			echo json_encode($response);
		}
		//viewdata fetch
		...
		if($_POST['action'] == 'ac_change_status')
		{
			if($_POST['id'])
			{
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Chat Room Status changed successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Chat Room Status NOT changed.';
				}
				echo json_encode($response);
			}
	  }
		...
		if($_POST['action'] == 'ac_delete')
		{
			if($_POST['id']){
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Chat Room Record deleted Successfuly...';
				} else {
					$response['status'] = 'error';
		 		...
		 		<?php
		 			...
		 			if($result->num_rows > 0){
		 				while($data = $result->fetch_array()) {
		 					...
		 					echo "<tr colspan='7'><td>No Chat Room found.</td></tr>";
		 				}
		 			?>
		 		</tbody>
		 	  </table>
						...
						<form id="register_form" method="POST" autocomplete="off">
						    <div class="modal-body">
								<div class="form-group row mb-4">
									<label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">From Id</label>
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										<div class="input-group-prepend">
										  <div class="input-group-text"> <i class="fas fa-map-marker-alt"></i> </div>
										</div>
										<input type="text" id="from_id" name="from_id" class="form-control" placeholder="From Id" required />
									  </div>
									</div>
								</div>
								<div class="form-group row mb-4">
									<label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">To Id</label>
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										<div class="input-group-prepend">
										  <div class="input-group-text"> <i class="fas fa-map-pin"></i> </div>
										</div>
										<input type="text" id="to_id" name="to_id" class="form-control" placeholder="To Id" required />
									  </div>
									</div>
								</div>
								<div class="form-group row mb-4">
