...
if(isset($_SESSION['admin_uid']))
{
	...
	$page_title = "Holiday";
	$sub_page_title = "Holiday";
	$msg_title = "Holiday";
	$THIS_TABLE = $master_holiday_table;
	...
	if(isset($_POST['action']))
	{
		...
		if($_POST['action'] == 'ac_add')
		{
			...
			$select_query = " select id from $THIS_TABLE where date='$date' and status!='E' ";
			$result = $con->query($select_query) or die(mysqli_error($con));
			if($result->num_rows == 0)
			{
				$insert_query = "insert into $THIS_TABLE(date,description,created_at,created_by,status)
												values('$date','$description','$created_at','$created_by','$status')";
				...
				if($inst_res === true ){
					...
					$response['message'] = 'Holiday Not Added!';
				}
			} else {
				$response['status'] = 'error';
				$response = 'Holiday Title Already exists!';
			}
		...
		if($_POST['action'] == 'ac_edit')
		{
			...
			$update_sql = "update $THIS_TABLE set date='$date',description='$description',updated_at='$updated_at',updated_by='$updated_by' where id='$id'";
			...
			if($up_res === true) {
				...
				$response['status'] = 'error';
				$response['message'] = 'Holiday are Not Updated!';
			}
			echo json_encode($response);
		}
		//viewdata fetch
		...
		if($_POST['action'] == 'ac_change_status')
		{
			if($_POST['id'])
			{
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Holiday Status changed successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Holiday Status NOT changed.';
				}
				echo json_encode($response);
			}
	  }
		...
		if($_POST['action'] == 'ac_delete')
		{
			if($_POST['id']){
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Holiday Record deleted Successfuly...';
				} else {
					$response['status'] = 'error';
		 		...
		 		<?php
		 			...
		 			if($result->num_rows > 0){
		 				while($data = $result->fetch_array()) {
		 					...
		 					echo "<tr colspan='7'><td>Holiday Not found.</td></tr>";
		 				}
		 			?>
		 		</tbody>
		 	  </table>
