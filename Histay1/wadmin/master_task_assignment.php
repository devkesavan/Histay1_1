...
if(isset($_SESSION['admin_uid']))
{
	...
	$page_title = "T.Assignment";
	$sub_page_title = "T.Assignment";
	$msg_title = "T.Assignment";
	$THIS_TABLE = $master_task_asssignments_table;
	...
	if(isset($_POST['action']))
	{
		...
		if($_POST['action'] == 'ac_add')
		{
			...
			$select_query = " select id from $THIS_TABLE where emp_id='$emp_id'and task_id='$task_id' and status!='E'";
			$result = $con->query($select_query) or die(mysqli_error($con));
			if($result->num_rows == 0)
			{
				$insert_query = "insert into $THIS_TABLE(emp_id,task_id,task_status,created_at,created_by,status)
												values('$emp_id','$task_id','$task_status','$created_at','$created_by','$status')";
		...
		if($_POST['action'] == 'ac_edit')
		{
			...
			$update_sql = "update $THIS_TABLE set emp_id='$emp_id',task_id='$task_id',task_status='$task_status',updated_at='$updated_at',updated_by='$updated_by' where id='$id'";
			$up_res = $con->query($update_sql);
			if($up_res === true) {
				$response['status'] = 'success';
				$response['message'] = 'Task Assignment updated successfully!';
			} else {
				$response['status'] = 'error';
				$response['message'] = 'Task Assignment are Not Updated!';
			}
			echo json_encode($response);
		}
		//viewdata fetch
		...
		if($_POST['action'] == 'ac_change_status')
		{
			if($_POST['id'])
			{
				...
				if($res === true ){
					$response['status'] = 'success';
					$response['message'] = 'Task Assignment Status changed successfully!';
				} else {
					$response['status'] = 'error';
					$response['message'] = 'Task Assignment Status NOT changed.';
				}
				echo json_encode($response);
			}
	  }
			...
			<script>
		 	...
		 	<div class="table-responsive">
		 	  <table class="table table-striped table-hover" id="save-stage" style="width:100%;">
		 		...
		 		<tbody>
		 		<?php
		 			$select_query = "SELECT em.id,em.name as employee_name,tk.id,tk.task_name,ta.*,pr.name as project_name FROM $THIS_TABLE ta 
		 							INNER JOIN $master_employees_table em ON em.id=ta.emp_id 
		 							INNER JOIN $master_task_table tk ON tk.id=ta.task_id 
		 							INNER JOIN $master_projects_table pr ON pr.id=tk.project_id
		 							WHERE ta.status!='E'";
		 			...
		 			if($result->num_rows > 0){
		 				while($data = $result->fetch_array()) {
		 					...
		 					if($data['task_status'] == "YTS") {
		 						$task_status = '<span class="badge badge-danger badge-shadow">YET TO START</span>';
		 					}else if($data['task_status'] == "WIP"){
		 						$task_status = '<span class="badge badge-primary badge-shadow">WORK IN PROGRESS</span>';
							}else if($data['task_status'] == "COM"){
		 						$task_status = '<span class="badge badge-success badge-shadow">COMPLETED</span>';
		 					}else if($data['task_status'] == "PAS"){
		 						$task_status = '<span class="badge badge-warning badge-shadow">PAUSED</span>';
		 					}else if($data['task_status'] == "DRP"){
		 						$task_status = '<span class="badge badge-primary badge-shadow">DROPPED</span>';
		 					}
		 					echo "<tr>
		 							<td>$data[employee_name]</td>
									<td><b>$data[project_name]</b> - $data[task_name]</td>
									<td>$task_status</td>
		 							<td>$data[created_at]</td>
		 							<td class='text-center'>$status</td>
		 							<td class='text-center'>
		 									<button type='button' class='btn btn-warning btn-sm add_view' data-id='$data[id]'><i class='far fa-edit'></i></button>
		 					...
		 					echo "<tr colspan='7'><td>No Task Assignment found.</td></tr>";
		 				}
		 			?>
		 		</tbody>
		 	  </table>
					  ...
					  <div class="modal-dialog modal-dialog-centered" role="document">
						...
						<form id="register_form" method="POST" autocomplete="off">
						    <div class="modal-body">
									...
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										...
										<option value=''>Select Task</option>
										 <?php
										 	$select_query = "SELECT pr.name as project_name,tk.* FROM $master_task_table tk 
										 						INNER JOIN $master_projects_table pr ON pr.id=tk.project_id 
										 						WHERE tk.status!='E'";
                                        	$result = $con->query($select_query) or die(mysqli_error($con));
                                        	while($data = $result->fetch_array())
                                        	{	
                                        		echo "<option value='$data[id]'>$data[project_name] - $data[task_name]</option>";
                                        	}
										 ?>
										</select> 
									  </div>
								...
								<div class="form-group row mb-4">
									<label class="col-form-label text-md-right col-12 col-md-3 col-lg-3">Task Assignments</label>
									<div class="col-sm-12 col-md-9">
									 <div class="input-group">
										"<p>
										<label><input type="radio" id="task_status_y" name="task_status" value="YTS" required /> Yet To Start</label>&nbsp&nbsp&nbsp
										<label><input type="radio" id="task_status_w" name="task_status" value="WIP" required /> Work In Progress</label>
										<label><input type="radio" id="task_status_c" name="task_status" value="COM" required /> Completed</label>&nbsp&nbsp&nbsp
										<label><input type="radio" id="task_status_p" name="task_status" value="PAS" required /> Paused</label>&nbsp&nbsp&nbsp
										<label><input type="radio" id="task_status_d" name="task_status" value="DRP" required /> Dropped</label>&nbsp&nbsp&nbsp
										</p>
									  </div>
									</div>
								</div>
