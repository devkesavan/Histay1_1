...
if(isset($_SESSION['admin_uid']))
{
	...
	$sub_page_title = "Property Type";
	$msg_title = "Property Type";
	$THIS_TABLE = $master_property_type_table;
		 		...
		 		<?php
		 			...
		 			if($result->num_rows > 0){
		 				while($data = $result->fetch_array()) {
		 					...
		 					if($data['des_image'] != "")
		 						$des_img = "<img src='../$pro_type_img_dir/small/$data[des_image]' style='width:100px;'>";
		 					else
		 						$des_img = "[NO IMAGE]";
		...
		$(document).ready(function (){
			...
			$(document).on('click','.edit', function(event){
				...
				success:function(data)
				{
					...
					$('#des_image_pre').attr("src", "<?php echo "../$pro_type_img_dir/small/"; ?>"+data.des_image).show;
					$('#id').val(id);
					$('#action').val('ac_edit');
					$('#save').val('Update');
					},error:function(xhr, ajaxOptions, thrownError){alert(xhr.responseText); ShowMessage("??? ?? ","fail");}
